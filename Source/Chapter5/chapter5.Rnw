...
A key principle of replicatable data gathering with R, like replicatable research in general is segmenting the process into discrete files that can be run by a common Make file.

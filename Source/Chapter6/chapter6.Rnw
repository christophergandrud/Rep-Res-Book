...
learn how to use \textbf{Dropbox} and \textbf{GitHub} for data:
...
\textbf{R} is able to read (and write) a very wide variety of file
formats, mostly through the \texttt{foreign} package in \texttt{base}
\textbf{R}. This includes
...
\textbf{Dropbox} and \textbf{Google Drive}. These services typically
...
control. I am going to focus on using \textbf{Dropbox} because it
currently offers a complete set of features that allow you to store,
version, collaborate, and access your data.

\subsection{Version control}

\textbf{Dropbox} has a simple version control system. Every time you
save a document on \textbf{Dropbox} a new version is created. One the
\textbf{Dropbox} website

\subsection{Accessing Data}

There are two similar, but importantly different ways to access data
stored on \textbf{Dropbox}. All files stored on \textbf{Dropbox} have a
...
\textbf{R}, while others must me manually (point-and-click) downloaded
onto your computer and then loaded into \textbf{R}. The key factor is
whether or not the files are located in your \textbf{Dropbox}'s
\emph{Public} folder. Files in the \emph{Public} folder can be
downloaded directly into \emph{R}. Files not in the \emph{Public} folder
...
file icon in you \emph{Dropbox} folder. If the file is stored in the
\emph{Public} folder, you go to \textbf{Dropbox} then \textbf{Copy
Public Link}. This copies the URL into your clipboard from where you can
paste it into your \textbf{R} source code (or wherever). Once you have
the URL you can load the file directly into \textbf{R} using the
\texttt{read.table} command for dataframes or the \texttt{source}
command for source files. This was covered in more detail in chapter
GET.

If the file is not in your \emph{Public} folder you also go to
\textbf{Dropbox} after right-clicking. Then choose \textbf{Get Link}.
...
\textbf{Dropbox}. First log into the \textbf{Dropbox} website. When you
...
files in the \textbf{Dropbox} folder on your computer.

\section{GitHub}

\textbf{Dropbox} does a fine job of meeting our four basic criteria for
reproducible data storage. \textbf{GitHub} meets these criteria and
more.

\textbf{GitHub} was not explicitly designed to host research projects or
even data. It was designed to host `socially coded' computer programs.
It built an interface on top of the \textbf{git} version control system
...
Because \textbf{GitHub} is very actively used by people with very
...
As is usually the case, \textbf{GitHub}'s added features mean that it is
takes a longer time than \textbf{Dropbox} to set up and become familiar.
So we need good reasons to want to invest the time needed to learn
\textbf{GitHub} rather than just sticking with \textbf{Dropbox} or a
similar service. Here is a list of \textbf{GitHub}'s key features
relative to \textbf{Dropbox} for reproducible research:

\begin{itemize}
\item
  \textbf{Git} is directly integrated into \textbf{RStudio} projects
  (\textbf{RStudio} also supports the \textbf{subversion} version
  control system, but I don't cover that here).
\item
  \textbf{Dropbox}'s version control system only lets you the see the
  file names, the times they were created, who created them, and revert
  back to specific versions. \textbf{git} tracks every change you make
  in a way that makes it relatively easy to find the version you want.
  The \textbf{GitHub} website and GUI programs for Mac and Windows
  provide nice interfaces for examining specific changes. You can also
  use the command line to see changes.
\item
  \textbf{Dropbox} creates a new version every time you save a file,
  which can make it difficult to actually find the version you want.
  \textbf{git}'s version control system only creates a new version when
  you tell it to.
\item
  \textbf{Dropbox} does not merge conflicting versions of a file
  ...
  \textbf{GitHub} identifies conflicts and lets you reconcile them.
\item
  The \textbf{GitHub} website as an `'Issues'' area where you can to
...
There are a number of ways to set up \textbf{GitHub} on your computer. I
...
\textbf{GitHub}'s version control system is much more comprehensive than
\textbf{Dropbox}'. However, it also has a steeper learning curve.

\paragraph{More Practice}

If you want more practice setting up \textbf{GitHub} in the command
line, \textbf{GitHub} and the website Code School have an interactive

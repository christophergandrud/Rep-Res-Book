...
If you are using **GitHub** or another service that uses secure URLs the steps are generally the same, but you need to use the `secure_url` command in the *devtools* package. For **GitHub** based source code find the file's URL the same way we did in Chapter 6. Remember to get the URL for the *raw* version of the file.  

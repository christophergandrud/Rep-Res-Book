...
{\small
<<Ch7SummaryExamp>>=
# Summarize AgMethaneData data frame object
summary(AgMethaneData)
@
}
...
# Summarize the methane emissions variable from AgMethaneData 
summary(AgMethaneData$EN.ATM.METH.AG.ZS)
@
...
<<Ch7MatrixDF>>=
# Convert LongPop to data frame
LongPopDF <- data.frame(LongPop)

...
You can view a portion of a data frame object with the \texttt{View} command.\index{R function!View} This will open a new window that lets you see a selection of the data frame. If you are using RStudio, you can click on the data frame in the \emph{Workspace} tab\index{RStudio!Workspace tab} and you will get something similar. Note that neither of these viewers are interactive in that you can't use them to manipulate the data. They are only data viewers. To be able to see similar windows that you can interactively edit use the \texttt{fix}\index{R function!fix} command in the same way that you use \texttt{View}. This can be useful for small edits, but remember that the edits are not reproducible.
...
\noindent You can select specific variables to keep with the \texttt{select} function by writing the variables' names without a minus sign. \textbf{Note} if you are merging many data sets it can sometimes be good to clean up duplicate columns between each \texttt{merge} call.

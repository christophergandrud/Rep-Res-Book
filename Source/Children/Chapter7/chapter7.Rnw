...
\noindent Note that this example is largely to demonstrate R syntax rather than to prescribe a certain transformation of skewed data with zeros. The choice of which transformation to make should ultimately be made based on the data, model, and context. See \cite{Hyndman2010} for more information on various alternatives including Box-Cox \citep{box1964analysis} and inverse hyperbolic sine transformations \citep{Burbidge1988}.

...
One thing to look out for after merging are duplicate observations. It is good to check for duplicates both before and after merging a data set. You can use the \texttt{duplicated}\index{duplicated} command to check for and remove duplicate observations using subscripts. For example, let's create a new object called \emph{MFSDuplicates} from the country-years that are duplicated in \emph{MoltenFertSub}. Note remember that \textbf{country} and \textbf{year} are in the second and third columns of the data frame.

<<Ch7Duplicated, tidy=FALSE>>=
# Created a data frame of unique country-years
MFSDuplicates <- MoltenFertSub[duplicated(
                                MoltenFertSub[, 2:3]), ]

# Show the number of rows in MFSDuplicates
nrow(MFSDuplicates)
@

\noindent In this data frame there are no duplicated country-year observations. This is indicated by the fact that using the \texttt{nrow}\index{nrow} command\footnote{\texttt{nrow} returns the number of rows an object has.} we find that the \emph{MFSDuplicates} data frame has no rows, i.e. no observations.
...
MFSNotDuplicated <- MoltenFertSub[!duplicated(
                                MoltenFertSub[, 2:3]), ]

# Show the number of rows in MFSNotDuplicated
nrow(MFSNotDuplicated)
@
...
Again a word of caution is in order. Lock over your data set and the source code that created the data set to try to understand why duplicates occurred. There may be a more fundamental problem in the way you are handling your data that resulted in the duplicated observations.

\subsection{Duplicate columns}

Another common post-merge clean up issue are duplicate variables. These are variables from the two data frames with the same name that were not included in \texttt{merge}'s \texttt{by} argument. If the variables were called \textbf{Variable1}, for example, \texttt{merge} would rename them \textbf{Variable1.x} and \textbf{Variable1.y} to signify which data set they are from. 

You should of course decide what to do with these variables on a case-by-case basis. But if you decide to drop one of the variables and rename the other, you can use subscripts (as we saw in Chapter \ref{GettingStartedRKnitr}). The \emph{gdata} package \citep{R-gdata} has a useful function called \texttt{remove.vars} that can also remove variables from data frames.

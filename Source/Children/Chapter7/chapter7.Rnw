...
MoltenFert <- rename(MoltenFert,
                    year = variable,
                    FertilizerConsumption = value)
...
We can order observations in our data set using the \texttt{order} command.\index{R function!order}\index{sort}\index{order} For example, to order \emph{MoltenFert} by country-year we type:
...
\emph{dplyr} has a function called \texttt{arrange}\index{R function!arrange} that can also be useful for ordering your data. \noindent \texttt{arrange}'s syntax is much cleaner and easy to remember for data frames than the operation we did with \texttt{order}. To arrange the \emph{MoltenFert} data as in the previous example, but with \texttt{arrange} use:

<<Ch7Arrange>>=
MoltenFert <- arrange(MoltenFert, country, year)
@

To arrange a variable in descending order simply place it in the \texttt{desc} function from \emph{dplyr}, e.g. \verb|arrange(MoltenFert, country, desc(year))|.\index{R function!desc}
...
The \texttt{cut} command \index{R function!cut} provides a less code intensive way of creating factors from numeric ones and labelling factor levels. For example:

<<Ch7Cut, tidy=FALSE>>=
# Create a factor variable with the cut command
FertFactor <- cut(MoltenFertSub$FertilizerConsumption,
                  breaks = c(-0.01, 14.99, 79.99, 149.99, 999.99),
...
Generally, the safest and most effective way to merge two data sets together is with the \texttt{merge}\index{merge}\index{R function!merge} command. Imagine that we want to merge our \emph{MoltenFertSub} data frame with two other data frames we created in Chapter \ref{DataGather}: \emph{FinRegulatorData} and \emph{DispropData}. The simplest way to do this is to use the merge command twice, i.e.:
...
\noindent Alternatively, you can select specific variables to keep with the \texttt{select} function by writing the variables' names without a minus sign. \textbf{Note} if you are merging many data sets it can sometimes be good to clean up duplicate columns between each \texttt{merge} call.

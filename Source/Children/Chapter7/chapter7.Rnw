...
We can order observations in our data set using the \texttt{order} command.\index{R command!order}\index{sort} For example, to order \emph{MoltenFert} by country-year we type:
...
\noindent You'll notice that we don't have a factor variable yet, our new variable is numeric. We can use the \texttt{factor}\index{R command!factor} to convert \emph{FertConsGroup} into a factor variable with the labels we want.
...
To create a data set of without duplicated observations (if there are duplicates) we just add an exclamation point (\texttt{!}) before \texttt{duplicated}--i.e. not duplicated--in the above code.\footnote{\texttt{!duplicated} is equivalent to the \texttt{unique} command.\index{R command!unique}}

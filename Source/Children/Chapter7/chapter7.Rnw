...
\noindent The first number is the number of rows in the data frame (\Sexpr{dim(FertConsumpData)[[1]]}) and the second is the number of columns (\Sexpr{dim(FertConsumpData)[[2]]}). You can alse use the \texttt{nrow} command to find just the number of rows and \texttt{ncol} to see only the columns.\index{R command!nrow}\index{R command!ncol}
...
The \texttt{cut} command \index{R command!cut} provides a less code intensive way of creating factors from numeric ones and labeling factor levels. For example:
...
\subsection{Changing variables types}\index{R!change variable type}
...
After merging data frames it is always a good idea to look at the result and make sure that the result of the merge are what you expected. Some post merging clean up may be required to get the data frame ready for statistical analysis.

\paragraph{Big data}\index{big data}

The \texttt{merge} command and many of the other data frame manipulation commands covered so far in this chapter may not perform well with very large data sets. If you are using very large data sets it might be worth investing time learning how to use SQL\footnote{Structured Query Language}\index{SQL}\index{Structured Query Language} or another special purpose data handling language.\footnote{w3schools has an online SQL tutorial at: \url{http://www.w3schools.com/sql/default.asp}.} Once you know how these languages work, you can incorporate them into your R workflow with a package like \emph{sqldf} \citep{R-sqldf}.\footnote{J.D. Long has a blog post on how to load large data sets into R using \emph{sqldf}: \url{http://www.cerebralmastication.com/2009/11/loading-big-data-into-r/} (posted 24 November 2009).}
...
\noindent In this data frame there are \Sexpr{nrow(DataDuplicates)} duplicated iso2c-year observations. This is indicated by the fact that using the \texttt{nrow}\index{Rcommand!nrow} command we find that the \emph{MergedData2} data frame has \Sexpr{nrow(DataDuplicates)} rows, i.e. \Sexpr{nrow(DataDuplicates)} observations.
...
A word of caution is in order. Look over your data set and the source code that created the data set to try to understand why duplicates occurred. There may be a fundamental problem in the way you are handling your data that resulted in the duplicated observations.
...
This chapter has provided you with many tools for cleaning up your data to get it ready for statistical analysis. Before moving on to the next chapter to learn how to do statistical analysis as part of a reproducible workflow with \emph{knitr} it's important to reiterate that the commands we've covered in this chapter should usually be embedded in the types of data creation files we saw in Chapter \ref{DataGather}. These files are then tied together with a makefile into a process that should be able to take very raw data and clean it up for use in your analyses. Embedding these commands in data creation source code files, rather than just typing the commands into your R Console\index{R!Console} or manually changing data in Excel will make your research much more reproducible. It will be easier to backtrack and find mistakes that you may have made while transforming the data. Including new or updated data when it becomes available will also be much easier if you use a series of segmented data creation source code files that are tied together with a makefile.

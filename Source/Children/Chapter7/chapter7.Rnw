...
The \texttt{cut} command \index{R function!cut} provides a less code intensive way of creating factors from numeric ones and labeling factor levels. For example:

...
The \emph{rmarkdown}\index{rmarkdown} package implements a variation on this process that utilizes Pandoc to create presentation documents in multiple formats. You simply begin with a knittable document written in Markdown. The key difference is that you include a header specifying how you want to process the document with Pandoc.

Note that the header is written in YAML\index{YAML}.\footnote{YAML is a recursive acronym that means ``YAML Ain't Markup Language''.} The YAML header can include information other than what type of output document you want to create including document's title, author, and a link to a BibTeX bibliography file among other options. YAML is a straightforward data format that organizes information in a simple hierarchy. The header begins and ends with three dashes (\texttt{---}). ``Keys''--like ``title'' and ``author''--are separated from their associated ``values'' by a colon (\texttt{:}). Subvalues of a hierarchy are denoted by being placed on a new line and indented.\footnote{It doesn't matter how many spaces you use to indent, as long as all indentations have the same number or spaces.} For example, this is a basic \emph{rmarkdown} header that indicates the document's title, author, date, and that it will be turned into a PDF document (via Pandoc and LaTeX).
...
RStudio can automatically create a basic header for the type of output document that you want. Simply select \texttt{File} \textrightarrow \texttt{New File} \textrightarrow \texttt{R Markdown\ldots}. A window will appear that looks like Figure \ref{rmarkdownWindow}. Simply select the type of output document you want to create and click \texttt{Ok}.

Remember that apart from the header, \emph{rmarkdown} documents are \emph{knitr} documents written in R Markdown syntax. You can also include Pandoc syntax if you like. This can be useful for bibliographies as we will discuss in Chapter \ref{MarkdownChapter}.

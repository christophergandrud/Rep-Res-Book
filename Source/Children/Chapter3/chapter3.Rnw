...
If we want to have an object with rows and columns and allow the columns to contain data with different types, we need to use data frames\index{data frame}. Let's use the \texttt{data.frame} command to combine the {\emph{NumericVect}} and {\emph{CharacterVect}} objects.\label{data.frame}
...
\begin{table}
  ...
  \begin{tabular}{l c p{5cm}}
    ...
    \texttt{fig.path} & Sets the directory where figures will be saved. \\[0.25cm]

...
<<Ch6ExampleMakeile, eval=FALSE, engine='sh'>>=
################
# Example Makefile
# Christopher Gandrud
# Updated 15 January 2012
...
\noindent To run the makefile simply type \texttt{make} into your shell. It will create the CSV final data file and three files with the extension \texttt{.Rout}, indicating when the segmented data gathering files were last run. Remember to make sure your current working directory is the one with the files you want to run.
...
The second line (\texttt{OUT_FILES := $(RSOURCE: .R = .Rout)}) creates a variable

...
UrlAddress <- paste0("https://raw.githubusercontent.com/",
                    "christophergandrud/Disproportionality_Data/",
                    "master/Disproportionality.csv")
...
load(file = "Children/Chapter6/FertData.RData")
@
...
\begin{itemize}
    ...
    \item Looping:\index{loop} Web scraping often involves applying a function to multiple things, e.g. tables or HTML tags. To do this in an efficient way you will need to use loops and apply functions\index{R!apply functions}. \cite{Matloff2011} provides a comprehensive overview. The \emph{plyr} package \citep{R-plyr}\index{plyr} and \citep{R-dplyr}\index{dplyr} (for data frames) are also particularly useful.
\end{itemize}

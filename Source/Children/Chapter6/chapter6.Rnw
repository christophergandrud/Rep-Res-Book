...
{\small{
<<Ch6sourceDataGitHub, tidy=FALSE>>=
DispropData <- repmis::source_data(UrlAddress)
@
}}

\noindent You can see that the file's SHA-1 hash begins \emph{\texttt{195637339e8483d}} \ldots. Let's see what happens when we try to download a slightly different version of the same file\footnote{The only difference is that the variable names begin with capital letters.} while placing this SHA-1 hash in \verb|source_url|'s \verb|sha1| argument. The URL of the alternative version of the file is in the object \emph{OldUrlAddress}:\footnote{See Section \ref{GitDownload} for the full URL.} 

{\footnotesize{
<<Ch6SHA1compare, tidy=FALSE>>=
DispropData <- repmis::source_data(OldUrlAddress, 
        sha1 = "195637339e8483dd634fae38e16ad8f24a403aef")
@
}}

\noindent If we set the \texttt{sha1} argument in our replication files others can be sure that they are using the same data files that we used to generate a result. This may not be practical while a piece of research is under active development, as the files may be regularly updated. However, it can be very useful for source code files that underlie published results.

\paragraph{Loading data from Dropbox non-Public folders with {\tt{source\_DropboxData}}}\label{DropboxNonPublic}

Files stored on Dropbox\index{Dropbox}\index{Dropbox!non-Public folders} non-\emph{Public} folders are a little trickier to download. If you go to the Dropbox website and click the \texttt{Share Link} button next to a file (\includegraphics[scale=0.5]{Children/Chapter5/images5/DropboxLink.png}) you will be taken to a new webpage. Notice that you get more than the raw file. The plain-text file is embedded in a larger HTML webpage. It is difficult to extract the data from this webpage. Luckily, \emph{repmis}\index{repmis} includes a \verb|source_DropboxData|\index{R command!source\_DropboxData} command for downloading data stored in a non-Public Dropbox folder into R. It works in much the same way as \verb|source_data|, the only difference is that instead of using the URL we need (a) the file's name and (b) it's Dropbox key.\index{Dropbox!key}
...
A more laborious way to download data from a secure URL that does not rely on \emph{repmis} is to use the \texttt{getURL}\index{getURL}\index{R command!getURL} command in the {\emph{RCurl}} package \cite[]{R-RCurl} as well as \verb|read.table|\index{R command!read.table} and \texttt{textConnection}.\index{R command!textConnection} The latter commands are in base R. The two rules about data being stored in plain text-formats and not being embedded in a larger HTML website apply to this method as well.

Let's try an example. To download the data file we used in Section \ref{GitDownLoad} it into R we could use this code:

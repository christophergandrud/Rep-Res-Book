...
There are more websites with APIs\index{API} than R packages designed specifically to access each one. If an API is available the \emph{httr} package \citep{R-httr}\index{httr}  may be useful. It is a wrapper\index{wrapper} for \emph{RCurl} intended to make accessing APIs easier. As of the time when I was writing the second edition of this book Hadley Wickham's \emph{rvest} \index{rvest} package was not on CRAN.\index{CRAN} However, it may turn out to be a useful addition to the R web scraping tool chest. More information is available at the package's GitHub site: \url{https://github.com/hadley/rvest}.

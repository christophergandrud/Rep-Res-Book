...
<<Ch6sourceDataGitHub, tidy=FALSE, size='small'>>=
...
<<Ch6httpsLoad, cache=TRUE, message=FALSE, tidy=FALSE, size='small'>>=
...
Let's download a compressed file called {\emph{uds\_summary.csv}} from \cite{Pemstein2010}. It's in a compressed file called {\emph{uds\_summary.csv.gz}}. At the time of writing, the file's URL address is {\url{http://www.unified-democracy-scores.org/files/20140312/z/uds_summary.csv.gz}}. I shortened the URL\footnote{I used bitly (\url{bitly.com}) to shorten the URL.} to \url{http://bit.ly/1jXJgDh} because of space constraints.

...
We can use the \verb|source_GitHubData| command in the \emph{repmis} package\footnote{Please see page \pageref{repmisInstall} for installation instructions. } \citep{R-repmis} to simplify the process of downloading data from secure URLs.\index{repmis}\index{R command!source\_GitHubData} To load the data stored at the URL in \emph{UrlAddress} with \verb|source_GitHubData| type: 

<<Ch6UsesourceGitHubData, eval=FALSE>>=
DispropData <- repmis::source_GitHubData(url = UrlAddress)
@

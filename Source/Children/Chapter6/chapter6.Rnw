...
To simplify the process of downloading data from secure URLs I've created a simple function called \verb|source_GitHubData|.\index{source\_GitHubData} It is available at: \url{http://bit.ly/UOMkpd}.\footnote{The non-shortened version of the URL is: \url{https://raw.github.com/christophergandrud/Rep-Res-Examples/master/Functions/source_GitHubData.R}.} Load the function with the \verb|source_url| command in the \emph{devtools} package. We will discuss this command in more detail in Chapter \ref{StatsModel} (page \pageref{SourceSecureURL}). To load the function type:

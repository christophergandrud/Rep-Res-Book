...
\paragraph{Loading data from secure URLs with {\tt{source\_GitHubData}}}

To simplify the process of downloading data from secure URLs I've created a simple function called \verb|source_GitHubData|.\footnote{The function is based on \texttt{source\_url} from \emph{devtools}\index{devtools} \citep{R-devtools}.}\index{R command!source\_GitHubData} It is available at: \url{http://bit.ly/UOMkpd}.\footnote{The full version of the URL is: \url{https://raw.github.com/christophergandrud/Rep-Res-Examples/master/Functions/source_GitHubData.R}.} Load the function with the \verb|source_url| command in the \emph{devtools} package. We will discuss \verb|source_url| in more detail in Chapter \ref{StatsModel} (page \pageref{SourceSecureURL}). To load the function type:

<<Ch6GitHubDataFunction, eval=FALSE>>=
# Load devtools
library(devtools)
...
\noindent Now we can just enter the data set's URL stored, as before, in an object called \emph{UrlAddress}. 

<<Ch6UsesourceGitHubData, eval=FALSE>>=
# Load disproportionality data
DispropData <- source_GitHubData(url = UrlAddress)
@
...
\begin{itemize}
    \item The \emph{openair} package \citep{R-openair}, which beyond providing a number of tools for analyzing air quality data also has the ability to directly gather data directly from sources such as Kings College London's London Air (\url{http://www.londonair.org.uk/}) database.\index{air quality}
    \item The \emph{quantmod} package \citep{R-quantmod} allows you to access data from Google Finance,\footnote{\url{http://www.google.com/finance}} Yahoo Finance\footnote{\url{http://finance.yahoo.com/}} and the US Federal Reserve's FRED\footnote{\url{http://research.stlouisfed.org/fred2/}} economic database.\index{finance data}
    \item The \emph{treebase} package by \cite{Boettiger2012} allows you to access phylogenetic\index{phylogenetic} data from TreeBASE.\footnote{\url{http://treebase.org}}
    \item The \emph{twitteR} package \citep{R-twitteR} accesses Twitter's\footnote{\url{https://twitter.com/}} API. This allows you to download data from Twitter\index{Twitter} including tweets\index{tweet} and trending topics.
    \item The \emph{WDI} package \citep{R-WDI} allows you to directly download data from the World Bank's\index{World Bank} Development Indicators database.\footnote{\url{http://data.worldbank.org/data-catalog/world-development-indicators}} This database includes numerous country-level economic, health, and environment variables. 

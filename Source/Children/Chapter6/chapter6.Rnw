...
We can use the \verb|source_GitHubData| command in the \emph{repmis} package \citep{R-repmis} to simplify the process of downloading data from secure URLs.\index{repmis}\index{R command!source\_GitHubData} To load the data stored at the URL in \emph{UrlAddress} with \verb|source_GitHubData| type: 

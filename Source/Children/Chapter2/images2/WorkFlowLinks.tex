...
\begin{tikzpicture}
    ...
    \node (pres) at (13, 5) [Links]{{\emph{knitr}} \\ \texttt{input} \\ \texttt{include} \\ \texttt{includegraphics} \\ \texttt{![]()}};
    \node (knitr) at (7.5, -1) [Links]{ {\emph{knitr}} \\ \texttt{source} \\ \texttt{source\_url} \\ \texttt{toLaTeX} \\ \texttt{print(xtable())} };
    \node (readData) at (3, -1) [Links]{ \texttt{read.table} \\ \texttt{read.csv} \\ \texttt{getURL} };
    
    \node (importData) at (-1, -1) [Links]{\texttt{read.table} \\ \texttt{read.csv} \\ \texttt{merge}\\ \texttt{getURL} \\ API-based packages};

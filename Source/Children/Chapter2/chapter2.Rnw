...
\noindent Chapter \ref{DirectoriesChapter} gives specific details about how to create files with dynamically included session information. If you use non-R tools you should also record what versions you used.
...
\noindent In this book I follow these guidelines when displaying code. He also suggests that all of your source code files should begin with a comment header. {\emph{At the least}} the header should include:
...
\begin{table}
    ...
    \begin{tabular}{p{2.5cm} c p{5.25cm} p{2cm}}
        ...
        \emph{rmarkdown} & R & R package that works generally like \emph{knitr}, but allows you to use Markdown to output to either HTML or PDFs compiled with LaTeX. & \hfill Throughout \\[0.25cm]

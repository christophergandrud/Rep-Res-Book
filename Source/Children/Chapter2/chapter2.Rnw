...
If you are like me, you will probably be tempted to start using the individual tools of reproducible research as soon as you find learn them. However, I would recommend stepping back and considering how the stages of reproducible research {\emph{tie}} together. This will make your reproducible research workflow more coherent from the beginning and save you a lot of backtracking later on.  Figure \ref{WorkflowTies} illustrates the workflow. Notice that the arrows connecting the workflow's parts point in both directions, indicating that we should always be thinking how to make it easier to go backwards through your research, i.e. reproduce it. 

Around the edges of the workflow are some of the commands you will learn to make it easier to go forwards and backwards through the process. These commands tie your research together. For example, we can use API-based R packages\index{API} to gather data from the internet. We can use the \texttt{merge} command to combine data gathered from different sources into on data set. The \index{read.table}\index{read.table} and \texttt{getURL}\index{getURL} commands can be used to bring this data set into our statistical analyses. The {\emph{knitr}} package then brings our analyses into the presentation documents. This includes the code we used, the figures we created, and, with the help of commands such as \texttt{print} and the {\emph{xtable}} package, tables of results. We can even tie multiple presentation documents together, by for example accessing the same figure for use in a LaTeX article and a Markdown created website with the \texttt{includegraphics}\index{includegraphics} and \texttt{![]()}\index{![]()} commands, respectively. We'll cover these commands in detail throughout the book. For a brief overview of the commands see Table \ref{TableTieCommands}. 
...
Nagler \citeyearpar[491]{Nagler1995} argued that to make your code understandable it is good practice to:

...
There are a number of R style guides. Most of them are similar to the Google R Style Guide\index{Google R Style Guide}.\footnote{See: \url{http://google-styleguide.googlecode.com/svn/trunk/google-r-style.html}.} Hadley Wickham also has a nicely presented R style guide.\footnote{You can find it at \url{https://github.com/hadley/devtools/wiki/Style}.} You may want to use the {\emph{formatR}}\index{R package!formatR} \cite[]{R-formatR} package to automatically reformat your code so that it is easier to read.
...
\begin{table}
    ...
    \begin{tabular}{p{2.5cm} c p{5.25cm} p{2cm}}
        ...
        {\tt{read.table}} & R & Reads a table into R. You can use this to import a plain-text file formated data into R. & \hfill\ref{DataGather} \\[0.25cm]

...
\begin{itemize}
    ...
    \item the {\emph{knitr}} package for R and other tools, including \textbf{command line programs} like GNU Make and Git version control, for dynamically tying your data gathering, analysis, and presentation documents together so that they can be easily reproduced,

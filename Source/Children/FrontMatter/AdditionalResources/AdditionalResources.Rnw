...
Many of the reproducible research tools discussed in this book are improving rapidly. Because of this I will regularly post updates to the content covered in the book at: \url{http://christophergandrud.github.io/RepResR-RStudio/errata.htm}.

\subsection*{Chapter Examples}

\addcontentsline{toc}{subsection}{Chapter Examples}

Longer examples discussed in individual chapters, including files to dynamically download data, code for creating figures, and markup files for creating presentation documents, can be accessed at: \url{https://github.com/christophergandrud/Rep-Res-Examples}. Throughout the book I refer to each file's specific URL. Please see Chapter \ref{Storing} for more information on downloading files from GitHub, where the examples are stored.\index{GitHub}

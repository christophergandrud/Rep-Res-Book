...
install.packages(c("apsrtable", "brew", "countrycode",
                ...
                "googleVis", "httr", "knitcitations", "knitr", 
...
In Chapter \ref{TablesChapter} we use the \emph{Zelig} package \citep{R-Zelig}\index{R package!Zelig} to create a simple Bayesian normal linear regression. For this to work properly you will need to install an additional package called \emph{ZeligBayesian} \citep{R-ZeligBayesian}. To do this type the following code into your R console: \label{ZeligBayes}\index{R package!ZeligBayesian}

...
On Linux you will need to install the \emph{RCurl} \citep{R-RCurl}\index{RCurl} and \emph{XML}\index{XML} \citep{R-XML} packages seperately. Use your Terminal\index{Terminal} to install these packages with the following code:

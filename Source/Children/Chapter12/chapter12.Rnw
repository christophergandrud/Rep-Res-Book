...
Imagine that we are using our the \emph{MainData} data set discussed in the previous chapters and we want to create a LaTeX document for each country displaying its average fertilizer consumption (\emph{FertilizerConsumption}). First let's create a \emph{brew} template document. This document will include all of our markup and code chunks we want in our \emph{knitr} document. There is one small difference from regular knittable documents: it will use \emph{brew} syntax for including information from the data subset. 

Text in a \emph{brew} template document is printed as is unless it is between \emph{brew}'s delimiters. These are:

\begin{itemize}
    \item \texttt{\textless{}\% . . . \%\textgreater{}}: R function inside the delimiters are run,
    \item \texttt{\textless{}\%= . . . \%\textgreater{}}: contents are printed.
\end{itemize}

\noindent In the following example we use the latter one. Here is our \emph{brew} template:
...
\begin{knitrout}
    ...
    \begin{kframe}
        \begin{alltt}
\textless{}documentclass{article}

{\LARGE \textless{}\%= COUNTRY \%\textgreater{}}

\textbackslash{}vspace\{1cm\}

The mean fertilizer consumption for \textless{}\%= COUNTRY \%\textgreater{}
is: \textbackslash{}Sexpr\{\hlfunction{mean}(\textless{}\%= X \%\textgreater{}, digits = 1)\} kilograms per hectare of arable land.
            \end{alltt}
        \end{kframe}
\end{knitrout}

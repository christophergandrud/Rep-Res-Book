...
When you have your child code chunks in your parent document set up, just \texttt{knit} the parent like you would any other knittable file. The knittable children will be knit and included every time you knit the parent document.
...
\begin{enumerate}
    ...
    \item Create a \texttt{function}\index{R command!function}\index{R!functions} called \emph{BatchReports} for subsetting\index{R!subset} the data, brewing it, and knitting it.
        ...
        \end{itemize}
    \item \texttt{lapply}\index{R command!lapply} allows us to run our \texttt{BatchReports} function separately for every value of the \emph{COUNTRY} vector. \texttt{BatchReport}'s \emph{Name} argument takes the value \emph{COUNTRY}. Note: it is important to end the \emph{BrewTemplate.Rnw} with a blank line for \texttt{lapply} to work correctly.

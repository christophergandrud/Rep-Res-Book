...
The most important \texttt{knitr} chunk option for showing tables is \texttt{results}\index{knitr option!results}.\label{ResultsOptions} The \texttt{results} option can have one of four values:
...
You can add element attributes\index{HTML element!attributes} inside of start tags.\footnote{These work like arguments in R in that they change how the element is evaluated.} For example, to add a border to the table use: \verb|<table border="1">|.\footnote{Whether or not a border appears is determined by whether or not the style sheet you are using includes borders.}\index{HTML!table borders}
...
Just as the \texttt{write.csv} command turns an R data frame into a CSV formatted text file, there are a number of methods in R to take an object--e.g. a matrix, data frame--the output from a statistical analysis and so on--and turn them into LaTeX and HTML tables. \texttt{kable}, \emph{xtable}, and \emph{texreg} each work most easily with specific object classes.
...
\texttt{kable} and \emph{xtable} are limited in when it comes to creating tables from statistical model objects. \texttt{kable} only works with matrices and data frames. \emph{xtable} is easiest when working with only one model object at a time. Furthermore, by default these tools do not create output tables that present estimates from multiple statistical models in the style used by many prominent academic journals. The \emph{texreg} package is very useful for creating these types of tables. It also supports more model object types than \emph{xtable}.

\paragraph{\emph{texreg} for LaTeX}
...
\noindent We can now include these model objects in one LaTeX table with \emph{texreg}.

<<Ch9BasictexregTable1,eval=FALSE>>=
# Load texreg package
library(texreg)
...
You can also use the \emph{texreg} package to create tables in HTML documents. Simply use the \texttt{htmlreg} function\index{R function!htmlreg} rather than \texttt{texreg}. The syntax is largely similar, though arguments relating to LaTeX are not available while others relating the HTML are.
...
\parapgraph{LaTeX scalebox for tables}\index{LaTeX!scalebox}

If your table is very wide then the \texttt{scalebox} function from the \emph{graphics} package\index{LaTeX!graphics package} could be useful. This expands or shrinks the text in the table. \texttt{texreg} actually has a \texttt{scalebox} argument. If you use \texttt{scalebox = 0.5} it will halve the size of the table; \texttt{scalebox = 2} doubles it.

More generally, to rescale a table use \verb|\scalebox{HORIZONTAL_SCALE}[VERTICAL_SCALE]{TABLE}|. \texttt{HORIZONTAL_SCALE} is how much to scale the table horizontally. \texttt{VERTICAL_SCALE} is how much to scale vertically and \texttt{TABLE} is the table or R code chunk to create the table.

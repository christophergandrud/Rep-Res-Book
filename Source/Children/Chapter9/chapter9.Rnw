...
\noindent In the first line we use the \texttt{names} command to create a vector of the \emph{swiss} data frame's column names. Then we simply create a vector of descriptions with the combine command (\texttt{c}).\index{R command!combine}\index{R command!c} Now we can combine these vectors into a matrix and use it to create an HTML table.

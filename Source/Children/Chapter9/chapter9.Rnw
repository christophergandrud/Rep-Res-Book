...
The goal of this chapter is for you to learn how to dynamically create tables for your presentation documents written in LaTeX and Markdown. We will first learn the simple \emph{knitr} syntax we need to dynamically include tables created from R objects. Then we will learn how to actually create the tables. There are a number of ways to turn R objects into tables that can be dynamically included in LaTeX or Markdown/HTML markup. In this chapter we mostly focus on the \texttt{kable} function in \emph{knitr}\index{R function!kable} and the \emph{texreg}\index{texreg}\index{R package!texreg} package \cite[]{R-texreg}. \texttt{kable} can create tables from data frames for both LaTeX and Markdown/HTML documents. \emph{texreg} produces publication quality tables from fitted model objects. It allows you to combine results from multiple models into one table. Unfortunately \emph{texreg} is less flexible with objects of classes it does not support.\footnote{These are not the only packages available in R for creating presentation document tables from R objects. I personally really like the \emph{stargazer} package \citep{R-stargazer}. It has a similar syntax to \emph{texreg} and is particularly good for showing results from multiple models estimated using different model types in one table.}
...
Just as the \texttt{write.csv} command turns an R data frame into a CSV formatted text file, there are a number of methods in R to take an object--e.g. a matrix, data frame--the output from a statistical analysis and so on--and turn them into LaTeX and HTML tables. In this section we will learn how to do this with the \emph{kable} function and \emph{texreg} package.

\subsection{\emph{kable} \& \emph{texreg} basics with supported class objects}

\emph{kable} and \emph{texreg} are fairly easy to use if you want to convert an object of a class that they support into a table. Let's first look at \emph{kable}.

\subsection{\texttt{kable} for Markdown and LaTeX}
...
\noindent This creates the Table \ref{kableExample1}:

\begin{center}
<<Ch9kableBasicRun,echo=FALSE, results='asis'>>=
kable(kable_ex, caption = "Example kable Table",
                label ="kableExample1")
@
\end{center}

\subsubsection{\emph{texreg} for LaTeX}\index{texreg}

The \texttt{kable} function is a very versatile tool for creating relatively well formatted tables from R objects. It only works with matrices and data frames. Furthermore, by default it is not set up to output tables that present estimates from multiple statistical models in the style used by many prominent academic journals. The \emph{texreg} package is very useful for creating these types of tables.
...
Imagine we want to show the estimates from a number of nested regression models in a table like Table \ref{Basic_texregTable}. For example, to estimate nested regression models from the remaining variables in the \emph{swiss} data set we would type:
...
\noindent We can now include these model objects in one table with \emph{texreg}.
...
\noindent Notice that we placed the fitted model objects in a list when we called \texttt{texreg}. \texttt{texreg} automatically created a the \texttt{table} environment and by default centers the table.\footnote{Use the \texttt{center = FALSE} argument to override this.} We added a caption and reference label with the \texttt{caption} and \texttt{label} arguments respectively. By default the caption is placed below the table, so we used \texttt{caption.above = TRUE} to place it on top.
...
NBSumDataFrame <- dplyr::rename(NBSumDataFrame, "2.5%" = X2.5.)
NBSumDataFrame <- dplyr::rename(NBSumDataFrame, "50%" = X50.)
NBSumDataFrame <- dplyr::rename(NBSumDataFrame, "97.5%" = X97.5.)

...
You can also change the markup language that the table is created in using the \texttt{format} argument. For example, to create a LaTeX formatted table use \texttt{format = 'latex'}. In general, you do not need to specify the format if you are using \emph{knitr} or \emph{rmarkdown} to include the table in a presentation document. This will be done automatically.

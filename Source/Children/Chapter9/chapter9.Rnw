...
\noindent We can now include these model objects in one LaTeX table with \emph{texreg}. Remember to include \texttt{results='asis'} in the code chunk head.

<<Ch9BasictexregTable1, results='asis'>>=
# Load texreg package
library(texreg)
...
You can also use the \emph{texreg} package to create tables in HTML documents. Instead of the \texttt{texreg} function use \texttt{htmlreg}\index{R function!htmlreg}. The syntax is largely similar, though arguments relating to LaTeX are not available while others relating the HTML are. Here is a simple example creating creating Table \ref{Basic_texregTable} in an HTML document:

<<Ch9htmlreg, eval=FALSE>>=

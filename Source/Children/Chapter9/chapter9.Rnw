...
The goal of this chapter is for you to learn how to dynamically create tables for your presentation documents written in LaTeX and Markdown. We will first learn the simple \emph{knitr} syntax we need to dynamically include tables created from R objects. Then we will learn how to actually create the tables. There are a number of ways to turn R objects into tables that can be dynamically included in LaTeX or Markdown/HTML markup. In this chapter we mostly focus on the \texttt{kable} function in \emph{knitr}\index{R function!kable} \cite[]{R-xtable} and \emph{texreg}\index{texreg}\index{R package!texreg} packages \cite[]{R-texreg}. \texttt{kable} can create tables for both LaTeX and Markdown/HTML documents. \emph{texreg} usually produces publication quality tables more easily than \texttt{kable}. Unfortunately it is less flexible with objects of classes it does not support.\footnote{These are not the only packages available in R for creating presentation document tables from R objects. I personally really like the \emph{stargazer} package \citep{R-stargazer}. It has a similar syntax to \emph{texreg} and is particularly good for showing results from multiple models estimated using different model types in one table.}
...
To find a full list of object classes that \emph{xtable} supports type \texttt{methods(xtable)}\index{R function!methods} into the R Console after you have loaded the package. To see \emph{apsrtable}'s supported classes type \verb|showMethods("modelInfo")|\index{R function!showMethods} into your console.
...
\textbackslash{}begin\{table\}
    \textbackslash{}caption\{Example Nested Estimates Table with \textbackslash{}emph\{texreg\}\}
    \textbackslash{}label\{BasictexregTableExample\}
        \textbackslash{}begin\{center\}

\textless{\textless}results='asis', echo=FALSE\textgreater{\textgreater}=
\hlcom{# Load texreg package}
\hlkwd{library}(texreg)

\hlcom{# Create nested regression model table}
\hlkwd{texreg}(list(M1, M2, M3, M4, M5))
@
...
\noindent Notice that we placed the fitted model objects in a list when we called \texttt{texreg}. In the LaTeX caption you'll notice \verb|\emph{texreg}|. In LateX the \texttt{emph}\index{LaTeX!emph} command italicizes text (we'll see this again in Chapter \ref{LatexChapter}). By default \texttt{texreg}: \verb|stars = c(0.001, 0.01, 0.05)| to determine at what p-values to display statistical significance stars. This is the same as the \texttt{lm} model summary default showing three sets of statistical significance stars.\index{significance stars} You can define the significance levels by assigning a different numeric vector to the \texttt{stars} argument.. To hide the stars altogether use \verb|stars=''|.
...
Finally, note that you need to include the \texttt{dcolumn}\index{dcolumn} package in your LaTeX preamble, i.e. \verb|\usepackage{dcolumn}| (see page \pageref{LaTeXPreamble} for a discussion of LaTeX preambles) to include the tables created by \emph{texreg}.

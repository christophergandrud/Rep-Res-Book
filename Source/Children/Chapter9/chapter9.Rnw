...
\subsection{\emph{texreg} for LaTeX}\index{texreg}
...
\noindent Notice that we placed the model objects in a list when we called \texttt{texreg}. \texttt{texreg} automatically created a the \texttt{table} environment and by default centers the table.\footnote{Use the \texttt{center = FALSE} argument to override this.} We added a caption and reference label with the \texttt{caption} and \texttt{label} arguments respectively. By default the caption is placed below the table, so we used \texttt{caption.above = TRUE} to place it on top.

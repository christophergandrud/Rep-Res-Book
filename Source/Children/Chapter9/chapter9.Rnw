...
The most important \texttt{knitr} chunk option for showing tables is \texttt{results}\index{results, knitr option}.\index{ResultsOption} The \texttt{results} option can have one of three values:

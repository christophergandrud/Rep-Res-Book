...
\texttt{cache.extra=list(file.info(Data.csv)$mtime, R.version}
@
...
The second condition enabled by \texttt{R.version}\index{R function!R.version} reruns the chunk whenever the R version or even operating system changes. If you only want to rerun the chunk when the version of R is different then use \texttt{R.version.string}.\index{R function!R.version.string}
...
\noindent R code included inline with \texttt{Sexpr} is evaluated using current R options.\index{R!global options} So if you want all of the output from \texttt{Sepxr} to be rounded to the same number of digits, for example, it might be a good idea to set this in a code chunk with R's \texttt{options} command.\index{R function!options} See page \pageref{ROptions} for more details.

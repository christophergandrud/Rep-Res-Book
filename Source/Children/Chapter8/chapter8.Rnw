...
Note that R code included inline with \texttt{Sexpr} is evaluated using current R options.\index{R global options} So if you want all of the output from \texttt{Sepxr} to be rounded to the same number of digits, for example, it might be a good idea to set this in a code chunk with R's \texttt{options} command.\index{options, R command} See page \pageref{ROptions} for more details.
...
Sample <- \hlfunction{rnorm}(n = 1000, mean = 5, sd = 2)

\hlcomment{# Save sample}
\hlfunction{save}(Sample, file = \hlstring{"Sample.RData"})
@
            \end{alltt}
        \end{kframe}
\end{knitrout}
...
\hlfunction{load}(file = \hlcomment{"Sample.RData"})

\hlcomment{# Create histogram}
\hlfunction{hist}(Sample)
@
            \end{alltt}
        \end{kframe}
\end{knitrout}

...
\paragraph{{\tt{include}}}\index{LaTeX command!include}
...
If you do echo your code in your presentation document you may want to manually format it in a particular way. By default \texttt{knitr} `tidies' the code using the \emph{formatR} package. To turn this off simply set the option \verb|tidy=FALSE|.

For example, this code chunk has \verb|tidy=FALSE| so that I could put the arguments on separate lines:
...
We will look at the \texttt{results} option in more detail in the next two chapters (see especially page \pageref{ResultsOptions}). However let's briefly discuss the option value \texttt{hide}. Setting \verb|results='hide'| is almost the opposite of \texttt{echo=FASLE}. Instead of showing the results of the code chunk and hiding the code, \texttt{hide} shows the code, but not the results.
...
If you want to store a code chunk's output for use later, rather than running the code chunk every time you compile your presentation document, set the option \texttt{cache=TRUE}. When you do this the first time the document is knitted the chunk will be run and the output stored in a subdirectory of the working directory called \emph{cache}. When the document is subsequently knitted the chunk will only be run if the code in the chunk changes or its options. This is very handy if you have a code chunk that is computationally intensive to run. The \texttt{cache} option is set to \texttt{FALSE} by default.
...
\noindent You can also use the {\emph{devtools}} command \verb|source_gist|\index{source\_gist} in a similar way to source GitHub Gists\index{GitHub!Gist}. Gists are a handy way to share code over the internet. For more details see: \url{https://gist.github.com/}.

\section{Computationally Intensive Analyses}

Sometimes you may want to include computationally intensive analyses that take a long time to run as part of a \emph{knitr} document. This can make compiling the document frustrating because it will take a long time. There are at least two solutions to this problem: the \texttt{cache} chunk option and Makfiles. We discussed Makefiles Chapter \ref{DataGather}, so lets look at how to work with the \texttt{cache} option. 

Setting \verb|cache=TRUE| for the code chunk that contains the analysis. This will only run the code chunk only when the code written in the chunk or the other chunk options change. This is a very easy solution the problem. Though it does have a major drawback: other chunks can't access objects created by the chunk or use commands from packages loaded in it. A solution to this is to (a) have packages loaded in a separate chunk and to save objects created by the cached chunk to a separate file RData file that can be loaded in later chunks (see page \pageref{RSave}).\footnote{It's true that when \texttt{knitr} caches a code chunk it saves the chunk's objects to an \emph{.RData} file. However, it is difficult to load this file directly because the file name changes every time the cached chunk is rerun.}  
Imagine that in a cached code chunk we create an object called \emph{Sample}. Then in a later code chunk we want to use the \texttt{hist}\index{hist} to create a histogram of the sample. In the cached code chunk we save \emph{Sample} to a file called \emph{Sample.RData}.
...
\noindent The later code chunk for creating the histogram would go something like this:\footnote{For reference, \emph{Sample} was created by using the \texttt{rnorm} command\index{rnorm} to take a random sample of size 1000 from a normal distribution with a mean of five and standard deviation of two.}

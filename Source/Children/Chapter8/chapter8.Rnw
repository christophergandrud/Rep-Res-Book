...
\begin{table}[ht]
    \caption{\emph{knitr} \texttt{engine} Values}
    \label{EngineOptions}
    \begin{center}
...
\noindent If the code chunk you want to cache creates many objects, but you only want to save a few of them you can use \emph{knitr}'s' \texttt{cache.vars} chunk option.\index{knitr option!cache.vars} Simply give it a character vector of the objects' names that you want to save. 
\vspace{2cm}

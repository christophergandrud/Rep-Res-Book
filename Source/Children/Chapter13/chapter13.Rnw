...
For the HTML version of the document we do not want a table of contents as we set \texttt{toc: no}. We specified a CSS\index{CSS} theme called Flatly for our HTML document using \verb|theme: "flatly"|. As of this writing, \emph{rmarkdown} has a built-in ability to use a range of CSS files for formatting from Bootswatch (\url{http://bootswatch.com/}). Alternatively, you can link to a custom CSS file with the \texttt{css} option. Use \texttt{html\_document} to see other options. Notice that we can use \texttt{no} and \texttt{yes} instead of \texttt{false} and \texttt{true}, respectively.

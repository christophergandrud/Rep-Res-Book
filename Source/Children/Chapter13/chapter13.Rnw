...
Ok, once we have R and RStudio set up so that they can use Pandoc we can create documents that take advantage of Pandoc's Markdown functions. I am only briefly going to discuss two of the most important for reproducible Markdown documents: footnotes and bibliographies with BibTeX. For the full range of Pandoc's abilities see: \url{http://johnmacfarlane.net/pandoc/README.html}.
...
We can add BibTeX citations with a small addition to the Pandoc call we use to render our Markdown file. Imagine we have a bibliography file called \emph{Example.bib} in the same directory as our \emph{Example.Rmd} file. We need to add the \verb|--bibliography| option to Pandoc like this:
...
{      
	system(paste("pandoc", shQuote(inputFile), "-o", 
				 shQuote(outputFile), "--bibliography
				 Example.bib"))
}
@
...
\noindent Pandoc uses \emph{natbib}\index{natbib} by default so the citation \verb|[@Donohue2009| will appear as (Donohue et al, 2009). See the Pandoc README site for more options.

\subsection{CSS style files and Markdown}

You can customize the appearance of your Markdown files with custom CSS files. 

\paragraph{CSS style files in RStudio}

\paragraph{Custom CSS files with Pandoc}\index{CSS and Pandoc}

You can add custom CSS files to Pandoc rendered Markdown files as well. To do this add the \emph|--include-in-header| an option to your \texttt{pandocToHTML} call. This will include the file you specify verbatim to the HTML files's header.

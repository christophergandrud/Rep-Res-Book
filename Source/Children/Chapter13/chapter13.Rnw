...
\noindent The \texttt{system}\index{R command!system} sends the following code to your computer's command line shell where it runs Pandoc. \texttt{shQuote} allows us to use quotes as strings that can be parsed by the shell.\index{shell}\index{command line} In this function the strings are the input and output file names. You can of course customize this code to change how Pandoc renders your documents. For example, we saw in the previous chapter how to use Pandoc to output a Markdown file to markup languages other than HTML. See page \pageref{PandoctoLaTeXExample} for an example and the Pandoc website for more details: \url{http://johnmacfarlane.net/pandoc/}. 
...
To get started with \emph{slidify} load the \emph{devtools}\index{devtools} package and install \emph{slidify} from GitHub\index{GitHub}.\footnote{As of when I wrote this (January 2012) \emph{slidify} was not available on CRAN\index{CRAN}.}
...
\noindent This will create a new directory in your Dropbox \emph{Public} folder.\index{Dropbox!Public folder} To get the URL address for the slideshow navigate to the folder and copy the public link for the \emph{index.html} file (see Chapter \ref{Storing} page \pageref{PublicLink} for more details).
...
In this chapter we have learned a number of tools for dynamically presenting our reproducible research on the web. Though LaTeX and PDFs will likely remain the main tools for presenting research in published journals and books for some time to come, choosing to also make your research available in online native formats can make it more accessible to general readers and allow you to take advantage of interactive tools like \emph{googleVis}. Before moving on to the concluding chapter lets quickly look at a simple example of a complete reproducible research project created using the tools covered in this book.

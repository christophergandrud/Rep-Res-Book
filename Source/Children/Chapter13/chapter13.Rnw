...
pandocToHTML <-  
	function(inputFile, outputFile) {      
	system(paste("pandoc", shQuote(inputFile), "-o", 
				 shQuote(outputFile), "--bibliography Example.bib"))
	}
@
...
If you want to create more complex websites with R Markdown and host them on GitHub you might want to look into Jekyll\footnote{\url{https://help.github.com/articles/using-jekyll-with-pages}}\index{Jekyll} or the slightly newer Ruhoh\index{Ruhoh} platform.\footnote{\url{http://ruhoh.com/}}  Jason Fisher has a useful blog post about how to combine Jekyll with knitr. See:\url{http://jfisher-usgs.github.com/r/2012/07/03/knitr-jekyll/} (posted 3 July 2012).

...
\chapter{Presenting on the Web and More with Markdown}\label{MarkdownChapter}
...
Here is an example of an R Markdown document that creates the map we saw in Chapter \ref{FiguresChapter}:\footnote{This code is available on GitHub at: \url{https://github.com/christophergandrud/Rep-Res-Examples/blob/master/RMarkdownExamples/ExampleKnitrMarkdown.Rmd}.}
...
Separating lines with a blank line places a blank line in the HTML document. End a line with two or more white spaces (  ) to create a new line that is not separated by a blank line.\index{Markdown!new line}

\subsection{Italics and bold}\index{Markdown!italicsn}\index{Markdown!bold}
...
Markdown is simple and easy to use. But being simple means that it lacks important functionality for presenting research results, such as footnotes and bibliographies. Also, customizing the look of a Markdown document is difficult. In this section we will learn how to overcome these limitations with Pandoc and CSS. Space constraints limit me from giving a full introduction to either of these tools, so we are mostly going to focus on how to set up R so that we can use Pandoc and custom CSS style files custom render HTML documents.

\subsection{Pandoc}\label{PandocDiscussion}\index{Pandoc}

With a little customization you can include footnotes and Bibtex\index{BibTeX} into R Markdown documents. When you click on the \texttt{Knit HTML} button in RStudio it runs two commands: \texttt{knitr} to convert your \emph{.Rmd} file to \emph{.md} and then \texttt{markdownToHTML}\index{R command!markdownToHTML} from the \emph{markdown} package \citep{R-markdown}. What we need to do is reset \texttt{markdownToHTML} or create a new function to run Pandoc.

The simplest way to do this is to create a new R function\index{R command!function} that runs markdown files created by \emph{knitr} to Pandoc in order to create an HTML file:\footnote{This function is modified from one on the RStudio website. See below for more details.}
...
\noindent The \texttt{system}\index{R command!system} sends the following code to your computer's command line shell where it runs Pandoc. \texttt{shQuote} allows us quotes a string that can be parsed by the sell. In this function the strings are the input and output file names. You can of course customize this code to change how Pandoc renders your documents. 

As we saw in the previous chapter, we can use Pandoc to output a Markdown file to markup languages other than HTML. You can modify the previous code to do this. See page \pageref{PandoctoLaTeXExample} for an example and the Pandoc website for more details: \url{http://johnmacfarlane.net/pandoc/}. 

Imagine that we have a file called \emph{Example.Rmd} in our working directory. This is who we knit it and render it with Pandoc:

<<Ch13knitrPandoc, eval=FALSE>>=
# knit file to Example.md
knitr::knit("Example.Rmd", "Example.md")

# Render with Pandoc to Example.html
pandocToHTML("Example.md", "Example.html")
@

In RStudio you can use this type of function rather than \texttt{markdownToHTML} when you click \emph{Knit HTML} by setting a new \texttt{option} in an invisible file called \emph{.Rprofile}. Changing this file allows you to alter how R works. Note: using Pandoc rather than \emph{markdownToHTML} from the \emph{markdown} package allows us to use footnotes and bibliographies and but because we also loose functions, like automatic MathJax support specific to \emph{markdownToHTML} from \emph{markdown}. It would take some work (not covered here) get the best of both worlds. Here is the code to add to the \emph{.Rprofile}.\footnote{This code is from the RStudio website: (\url{http://www.rstudio.com/ide/docs/authoring/markdown_custom_rendering}.}
...
\paragraph{Footnotes with Pandoc}

Ok, once we have R and RStudio set up so that they can use Pandoc we can create documents that take advantage of Pandoc's Markdown extensions.\index{Pandoc!Markdown Extensions} Pandoc Markdown extensions add functions to regular Markdown syntax. I am only briefly going to discuss two of the most important for reproducible Markdown documents: footnotes and bibliographies with BibTeX. For the full range of Pandoc's abilities see: \url{http://johnmacfarlane.net/pandoc/README.html}.

To create a what Pandoc calls an ``inline note'' use the \verb|^[NOTE]| syntax. For example:

<<Ch13PandocInlineNotes, eval=FALSE, tidy=FALSE, engine='sh'>>=
This is some text.^[This is an inline footnote.]
@

\noindent When rendered a hyperlink number will appear after ``text.''. It will link to the note at the bottom of the page.

\paragraph{Bibliographies with Pandoc}

We can add BibTeX citations with a small addition to the Pandoc call we use to render our Markdown file. Imagine we have a bibliography file called \emph{Example.bib} in the same directory as our \emph{Example.Rmd} file. We need to add the \verb|--bibliography| option\index{Pandoc option!bibliography} to Pandoc like this:
...
This is a citation [@Donohue2009].
@

\noindent Pandoc uses \emph{natbib}\index{LaTeX package!Natbib} by default so the citation \verb|[@Donohue2009]| will appear as (Donohue et al, 2009). To add text before and after the citation inside of the parentheses use something like this: \verb|[see @Donohue2009, 10]|. This will create an in-text citation that looks like this: (see Donohue et al. 2009, 10) To include only the year and not the authors' surnames add a minus sign, e.g. \verb|[-@Donohue2009]|. See the Pandoc README site for more options.

Here is complete example of an R Markdown document that includes footnotes and bibliographies.
...
\noindent Note you can find is file and the bibliography plus commands to render the document at: \url{https://github.com/christophergandrud/Rep-Res-Examples/tree/master/RMarkdownExamples/ExamplePandocKnitrMarkdown}.

\subsection{CSS style files and Markdown}

You can customize the appearance of your Markdown files with custom CSS style sheets. CSS files allow you to specify the way a rendered Markdown file looks in a web browser including fonts, margins, background color, and so on. We don't have space to cover CSS syntax here. There are numerous online resources for learning CSS. One of the best ways may be to just copy RStudio's default CSS style sheet into a new file and play around with it to see how things change. One really good tool for this Google Chrome's Developer Tools.\index{Google Chrome!Developer Tools} This allows you to edit your webpages, including their CSS and have a live preview. It is a really nice way to experiment CSS (and HTML, JavaScript, and so on).  For more information on how to access and use Developer Tools in Chrome see: \url{https://developers.google.com/chrome-developer-tools/}. There are also numerous pre-made style sheets online.\footnote{One small note: when you create a new style sheet or copy an old one make sure the final line is blank. Otherwise R will give you an ``incomplete final line'' error.} In this example I will use a stylsheet called \emph{Markdown.css} created by and can be found at: \url{https://github.com/simonlc/Markdown-CSS}. 

\paragraph{Rendering Markdown files using custom CSS}\index{Markdown!custom CSS}

It is simple to use a custom CSS file with the \texttt{markdownToHTML} command. Just add save your custom \emph{.css} file in the working directory and use the \texttt{stylesheet} argument with \texttt{markdownToHTML}. Imagine we want to \texttt{knit} our \emph{Example.Rmd} file using a custom style sheet called \emph{Markdown.css}. Both files are in the working directory.
...
You can of course also modify your \emph{.Rprofile} so that you always use the same CSS style file. RStudio suggests adding the following code to your \emph{.Rprofile}:
...
\noindent Simply change the style sheets name and file path as needed.

\paragraph{Custom CSS files with Pandoc}\index{Pandoc!CSS}

You can add custom CSS files to Pandoc rendered Markdown files as well. To do this add the \verb|--css| option to your \texttt{pandocToHTML} call.\index{Pandoc option!css option} This will tell Pandoc what CSS file you want to use. For example:
...
There are a number of steps to create an HTML5 slideshow with \emph{Slidify}:

\begin{itemize}
    \item Initialize a slideshow with the \verb|author| command,
    \item Edit the slideshows main R Markdown file, called \emph{index.Rmd} by default. This includes both the slideshows header and body.
...
\begin{table}
    ...
    \begin{tabular}{l c}
        ...
        dzslides & \url{http://paulrouget.com/dzslides/} \\[0.25cm]
        html5slides & \url{http://code.google.com/p/html5slides/} \\[0.25cm]
        shower & \url{https://github.com/shower/shower} \\[0.25cm]
...
To get started with \emph{Slidify} load the \emph{devtools} packages and install \emph{Slidify} from GitHub\index{GitHub}.\footnote{As of when I wrote this (January 2012) Slidify was not yet available on CRAN\index{CRAN}.}
...
Use the \verb|author| command to create a new slideshow. Imagine we want to create a new slideshow in the \emph{Presentation} folder of our \emph{ExampleProject} called \emph{MySlideShow}. To do this type:
...
\emph{Slidify} R Markdown documents use very similar syntax to ordinary R Markdown documents. \emph{knitr} code chunks are written in the same way as regular R Markdown. An important difference is that three dashes (\verb|---|) delimit individual slide frames. Importantly, you need to have an empty line before and after the three dashes or else a new slide will not be created. Two hash marks (\verb|##|) are used to indicate a slide's title.\footnote{One hash mark creates a slide title formatted in the same way as the text.} 

\paragraph{Compiling a slideshow}

Use the \verb|slidify| command to compile an R Markdown file into a slideshow. This code runs \emph{knitr} and renders our R Markdown file into a slideshow:
...
\noindent In RStudio\index{RStudio} you can click the \texttt{Knit HTML} button and it will \texttt{slidify} the R Markdown file. Note that the resulting slideshow will likely not work in the RStudio \textbf{HTML Preview} window, but opening the \emph{index.html} file in your web browser should work fine. 
...
To publish our example \emph{MySlideShow} on GitHub first create a new GitHub repository\index{GitHub!repository} called `MySlideShow' (see Chapter \ref{Storing} page \pageref{NewGitHubRepo} for instructions on how to create a new repository). Make sure the repository is empty, i.e. has no files in it. Then type in R:
...
\noindent This will create a new directory in your Dropbox \emph{Public} folder. To get the URL address for the slideshow navigate to the folder and copy the public link for the \emph{index.html} file (see Chapter \ref{Storing} page \pageref{PublicLink} for more details).
...
We saw in the previous section how \emph{slidify} can publish slideshows via RPubs, Dropbox and GitHub. In Chapter \ref{GettingStartedRKnitr} (page \pageref{PublishRPubs}) we saw how to publish other R Markdown documents compiled with RStudio to RPubs. In this section we will look at how to publish general R Markdown documents using Dropbox and GitHub.

\subsection{Stand alone HTML file}

Before looking at the specifics of ways to publish R Markdown documents on the internet it's important to note that you can of course simply open the HTML file rendered from any R Markdown document in your web browser. If the HTML file contains the full information for the page, e.g. there are not auxiliary files it depends on, you can simply share this file and anyone with a web browser can open it. We can of course also send auxiliary files if need be, but this can get unwieldy.

\subsection{Hosting webpages with Dropbox}\index{Dropbox!Public folder}

Probably one of the easiest ways to host an HTML file created with R Markdown is on your Dropbox \emph{Public} folder.\footnote{See page \pageref{EnablePublicFolder} for instructions on how to enable this folder if you created your Dropbox account after 4 October 2012.} Just like we saw with \emph{Slidify} slideshows in the \emph{Public} folder any HTML file will be properly rendered and widely accessible simply be entering the public link into a web browser. 

\section{GitHub Pages}\index{GitHub!Pages}

GitHub also offers a free hosting service for webpages. These can be much more complex. The simplest way to create one of these pages is to create a repository with a file called \emph{README.Rmd}. You can \texttt{knit} this file and then create your GitHub Page with it. To do this go to the \texttt{Settings} \textrightarrow{} \texttt{GitHub Pages} on your repository's main GitHub website. Then click \emph{Automatic Page Generator}. This place the contents of your \emph{README.md} file in the page and provide you with formatting options. Click \texttt{Publish} and you will have a new website. 

Clicking \texttt{Publish} creates a new orphan branch\footnote{An orphan branch is a branch with a different root from other repository branches. Another way of think about this is that they have their own history.}\index{Git!orphan branch} called \emph{gh-pages}.\index{GitHub!gh-pages branch} When these branches are pushed to GitHub will create a website if they contain a file called \emph{index.html}. This will be the websites main page. 

If you want to create more customized and larger websites with GitHub pages you can manually create a GitHub Pages orphan branch and push it to GitHub. This is essentially what \emph{Slidify} did for us with its \texttt{publish} command. Imagine have our working directory set as a repository containing an R Markdown file that we have rendered into an HTML file called \emph{index.html}. Let's create a new orphan branch and remove all files from it:
...
\noindent Now \texttt{add} the files, \texttt{commit} the changes and \texttt{push} it to GitHub. Remember to push it to the gh-pages branch like this:
...
\noindent A new webpage will be created at: \emph{USERNAME.github.com/REPO\_Name} You can also add custom domain names. For details see: \url{https://help.github.com/articles/setting-up-a-custom-domain-with-pages}.

\paragraph{GitHub with Jekyll and Ruhoh}

If you want to create more complex websites with R Markdown and host them on GitHub you might want to look into Jekyll\footnote{\url{https://help.github.com/articles/using-jekyll-with-pages}}\index{Jekyll} or the slightly newer Ruhoh\index{Ruhoh} platform.\footnote{\url{http://ruhoh.com/}}  Jason Fisher has a useful blog post about how to combine Jekyll with knitr. See:\url{http://jfisher-usgs.github.com/r/2012/07/03/knitr-jekyll/} (posted 3 July 2012). And of course, because R Markdown creates HTML markup files you can use virtually any other web hosting service to make your presentation documents widely available.

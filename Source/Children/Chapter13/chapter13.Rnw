...
\section{Expanding Markdown with Pandoc and Custom CSS in RStudio}
...
With a little customization you can include footnotes and Bibtex\index{BibTeX} into R Markdown documents. When you click on the \texttt{Knit HTML} button in RStudio it runs two commands: \texttt{knitr} to convert your \emph{.Rmd} file to \emph{.md} and then \texttt{markdownToHTML}\index{markdownToHTML} from the \emph{markdown} package \citep{R-markdown}. What we need to do is reset \texttt{markdownToHTML} or create a new function to run Pandoc.

The simplest way to do this is to create a new R function\index{function, R command} that runs markdown files created by \emph{knitr} to Pandoc in order to create an HTML file:\footnote{This function is modified from one on the RStudio website. See below for more details.}
...
\noindent The \texttt{system}\index{system, R command} sends the following code to your computer's command line where it runs Pandoc. \texttt{shQuote} allows us quotes a string that can be parsed by the sell. In this function the strings are the input and output file names. You can of course customize this code to change how Pandoc renders your documents. 
...
\paragraph{Rendering Markdown files using custom CSS}\index{CSS, custom Markdown}
...
\paragraph{Custom CSS files with Pandoc}\index{CSS and Pandoc}

You can add custom CSS files to Pandoc rendered Markdown files as well. To do this add the \verb|--css| option to your \texttt{pandocToHTML} call.\index{--css, Pandoc option} This will tell Pandoc what CSS file you want to use. For example:
...
To publish our example \emph{MySlideShow} on GitHub first create a new GitHub repository\index{GitHub repository} called `MySlideShow' (see Chapter \ref{Storing} page \pageref{NewGHRepo} for instructions on how to create a new repository). Make sure the repository is empty, i.e. has no files in it. Then type in R:
...
Clicking \texttt{Publish} creates a new orphan branch\footnote{An orphan branch is a branch with a different root from other repository branches. Another way of think about this is that they have their own history.}\index{orphan branch, GitHub} called \emph{gh-pages}.\index{gh-pages} When these branches are pushed to GitHub will create a website if they contain a file called \emph{index.html}. This will be the websites main page. 

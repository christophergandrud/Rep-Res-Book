...
To get in line equations simply use parentheses instead of square brackets as in LaTeX, e.g. \verb|\( s^{2} = \frac{\sum(x - \bar{x})^2}{n - 1} \)|.
...
The easiest, but less customizable way to create HTML slideshows is with RStudio's new R Presentation documents.\footnote{This feature is only available from RStudio version 0.98.} 

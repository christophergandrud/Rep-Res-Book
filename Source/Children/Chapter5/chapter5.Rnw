...
To merge\index{Git command!merge} changes you commit in the \emph{Test} branch to the \emph{master} first checkout the \emph{master} branch then use the \texttt{merge}\index{merge}\index{Git!merge} command.
...
\subsubsection{Remote Storage on GitHub}\label{NewGitHubRepo}

...
\section{Storing Your Files in the Cloud: GitHub}\index{GitHub}
...
\noindent \verb|source_data| download the most recent version of the file from the master branch.

One very nice feature GiHub has for reproducible research is that you can download a particular version of a file--from a particular git commit--directly into R. This makes reproducing a specific result much easier. To do this you just need to use the a files raw URL from a particular commit. To find a file's particular commit raw URL first click on the file on GitHub's website. Then click the \texttt{Hitory} button (\includegraphics[scale=0.3]{Children/Chapter8/images8/GitHistory.png}). This will take you to a page listing all of the file's versions. Click on the \texttt{Browse Code} button (\includegraphics[scale=0.5]{Children/Chapter8/images8/BrowseCode.png}) next to the version of the file that you want to use. Click on the \texttt{Raw} button to be taken to the text-only version of the file. Finally, use this URL with \verb|source_data|. 
...
OldUrlAddress <- paste0("https://raw.github.com/",
					"christophergandrud/",
...
You can see in this example that the file's URL is similar to the one we saw previously. However instead of \verb|master| after \verb|Disproportionality_Data| we instead have this strange series of number and letters: \verb|1a59d360b36ea| \ldots. This is the commit's SHA-1 hash.\index{SHA-1 hash}

As we will see in Chapter \ref{StatsModel} (Section \ref{sourceurl}) you can also easily run source code files in R directly downloaded from GitHub with the \verb|source_url| command.

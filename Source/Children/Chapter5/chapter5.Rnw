...
In addition to versioning the data and analysis files that you create we also need to keep track of what versions of R packages we used to create our data and conduct our analyses. The very active user-generated package community is one of R's strengths. New features are regularly being added. However, frequent package updates can easily `break' your code, especially when functions' syntax change. This can lead to frustration when reproducing a piece of research and collaborating as others may have different package versions installed.

The most basic way to address this is to record your session info (as we did in Chapter \ref{GettingStartedRR}, page \pageref{SessionInfoHow}). It could nonetheless be tedious for others to hunt down specific package versions and install them (if they are even still available). The \emph{packrat} \citep{R-packrat}\index{packrat}\index{dependency management} package helps overcome this problem by manageming your package dependencies. It stores specific package versions in your project's directory. This can be shared over the cloud with collaborators or would-be reproducers. \emph{packrat} is closely integrated into RStudio.
...
If you are using a GitHub repo that is associated with a remote repository on GitHub,\index{GitHub} you can push and pull it with the \texttt{Pull Branches}\index{Git command!pull} and \texttt{Push Branch}\index{Git command!push} buttons in Git menu bar (the blue and green arrows respectively). You can also use the same buttons in the \textbf{Review Changes} window. The \emph{Git} tab also allows you to change branches, revert to previous commits, add files to \verb|.gitignore| and view your commit history. You can always use the \texttt{More \textrightarrow{} Shell \ldots} option to open a new shell with the Project set as the working directory to complete any other Git task you might want to do.

\index{GitHub|)}\index{Git|)}

\section{Package Management with \emph{packrat}}\index{dependency management|(}\index{packrat|(}

The \emph{packrat} package helps you manage your R package dependencies so that you, your collaborators, and would-be reproducers have exactly the versions of the packages you used to gather your data and analyze it. This helps avoid frustrating situations where your code breaks when you update a package.

To initialize \emph{packrat}, set your project's main directory as your working directory.

\subsection{Using \emph{packrat} in RStudio Projects}

You can initialize \emph{packrat} in a new RStudio Project. Simply click the \texttt{Use packrat with this project} dialog box when you create your project (see Figure \ref{NewProjectNewDirectory}).

When you have \emph{packrat} in the Project directory (or current working directory) [FILL IN]

\index{dependency management|)}\index{packrat|)}

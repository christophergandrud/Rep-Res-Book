...
\noindent Not, Git will not ignore files that have already been commited to a repository. To ignore these files you will first need to remove them from Git with Git's \texttt{rm} (remove) command.\index{rm, Git command} If you wanted to remove a file called \emph{ExampleProject.tex} from version tracking so that it could be ignored type:

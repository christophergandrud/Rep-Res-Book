...
The full LaTeX document example I showed you earlier uses the \texttt{LoadandCite} command\index{R command!LoadandCite} from the \emph{repmis} package. This simplifies the process of loading and citing R packages.\index{repmis}\footnote{It can also install the packages if the option \texttt{install = TRUE}.} 

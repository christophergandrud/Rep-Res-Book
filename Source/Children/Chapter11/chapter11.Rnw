...
\noindent The \emph{knitr} code chunk\index{knitr!code chunk} syntax should be familiar to you from previous chapters, so let's unpack the LaTeX syntax from just after the first code chunk. New syntax shown in later parts of this example is discussed in the remainder of this section and the next section on bibliographies.
...
\noindent In the first executable line we just create our list of packages to load and cite. The next command is \texttt{lapply}\index{R command!lapply} (list apply). This applies the function \texttt{library}\index{R command!lapply} to all of the items in \emph{PackagesUsed}. \texttt{character.only = TRUE} is a \texttt{library} argument that allows us to use character string versions of the package names as R sees them in the \emph{PackagesUsed} vector, rather than as objects (how we have used \texttt{library} up until now). If you include these commands in a code chunk at the beginning of your knitted document then you can be sure that you will have a BibTeX file with all of your packages. 

In the full LaTeX document example I showed you earlier there is a function (\texttt{LoadandCite})\index{R command!LoadandCite} that I created to simplify the process of loading and citing packages.\footnote{It can also install the packages if the option \texttt{install = TRUE}.} It available on the Book's GitHub Example repository and can be downloaded into R: i.e. \verb|devtools::source_url("http://bit.ly/UI95Oq")|.}

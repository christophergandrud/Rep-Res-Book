...
For basic LaTeX documents, such as short articles or simple presentations, it may often be quicker and simpler to write the markup using an R Markown document and compile it to PDF with the \emph{rmarkdown} package.\index{rmarkown} As we will see in Chapter \ref{MarkdownChapter}, Markdown syntax is much simpler than normal LaTeX. However, there are at least two reasons why it is useful to become familiar with LaTeX syntax. First, understanding LaTeX syntax will help you debug issues you might encounter when using \emph{rmarkdown} that would otherwise be mysterious if you were only familiar with Markdown. Second, R Markdown has limited capabilities for creating more complex documents such as books or documents with highly customizable formatting. Using \emph{kntr} and LaTeX can be useful in these situations.
...
\hlkwd{ggplot}(UDSData, \hlkwf{aes}(x = year, y = mean)) +

...
It is good practice to dynamically include the system information for the R session you used to create the project. To do this you can write your README file with R Markdown. Simply include the \texttt{sessionInfo()}\index{R command!sessionInfo} command in a \emph{knitr} code chunk in the R Markdown document. If you knit this file immediately after knitting your presentation document it will record the information for that session.
...
You can use \texttt{cat} to also print the contents of a one or more objects to a file. \textbf{Warning:} The \texttt{cat} command will overwrite existing files with the new contents. To add the text to existing files use the \texttt{append = TRUE} argument.
...
<<Ch4cdPowerShell, eval=FALSE, engine='sh'>>=
cd C:/Users/Me/Desktop
@

\noindent If you are in a child directory and want to change the working directory to the previous working directory simply type:

<<Ch4ChildtoParent, eval=FALSE, engine='sh'>>=
cd -
@

\noindent If, for example, our current working directory is \emph{/User/Me/Desktop} and we typed \texttt{cd -} then the working directory would change to \emph{/User/Me}. Note this will not work in PowerShell.
...
\noindent As we saw earlier, R also has an \texttt{ls} command.\index{R command!ls} R's \texttt{ls} command lists items in the R workspace. The shell's \texttt{ls} command lists files and directories in the working directory.
...
\noindent If you add the greater than symbol (\verb|>|) after the text you want to print and then a file name, \texttt{echo} will create the file (if it doesn't already exist) in the current working directory then print the text into the file.

<<Ch4Echo2, eval=FALSE, engine='sh'>>=
echo Reproducible Research with R and RStudio > ExampleEcho.md
@

\noindent Using only one greater than sign will completely erase the \emph{ExampleEcho.md} file's contents and replace them with \texttt{Reproducible Research with R and RStudo}. To add the text at the end of an existing file use two greater than signs (\verb|>>|).
...
\paragraph{{\tt{system}} (R command)}\index{R command!system}\label{systemRcommand}\index{R!shell commands}
...
The RStudio \emph{Files} pane allows us to navigate and do some basic file manipulation. The left panel of Figure \ref{FilesPane} shows us what this pane looks like. The pane allows us to navigate to specific files and folder, delete and rename files. To select a folder as the working directory tick the dialog box next to the file then click the \texttt{More} button and select \texttt{Set As Working Directory}. Under the \texttt{More} button (\includegraphics[scale=0.5]{Children/Chapter4/images4/MoreIcon.png}) you will also find options to \texttt{Move} and \texttt{Copy} files (see the right pane of Figure \ref{FilesPane}.
...
In this chapert we've learned how to organize our research files to enable dynamic replication. This included not only how they can be ordered in a computer's file system, but also the file path naming conventions--the addresses--computers uses to locate files. Once we know how these addresses work we can use many R and shell commands to refer to and manipulate our files. This skill is particularly useful because it allows use to place code in text-based files to manipulate our project files in a highly reproducible way. In the next few chapters we will begin to put these skills in practice when we learn how to store our files and gather data in reproducible ways.

...
To download a full (though very short) example of a reproducible research project created using the tools covered in this book go to: \url{https://github.com/christophergandrud/Rep-Res-ExampleProject1}. Please follow the replication instructions in the main \emph{README.Md} file. Before doing that, lets get a quick overview of the files. Figure \ref{ExampProjeFiles} shows how the files are organized in the file system. Figure \ref{ExampProjDiagram} illustrates how the main files are dynanically tied together. 

...
Understanding how \emph{knitr} dynamically includes figures is easier if you understand how figures are normally included in LaTeX and Markdown. Unlike a word processing program like Microsoft Word\index{Microsoft Word}, in LaTeX, Markdown, HTML, and other markup languages you don't copy and paste figures into your document. Instead you link to an image file outside of your markup document. Typically these images files are in formats such as PDF, EPS, PNG, and \emph{JPEG}.\index{PDF}\index{EPS}\index{PNG}\index{JPEG}\footnote{PDF: Portable Document Format, EPS: Encapsulated PostScript, PNG: Portable Network Graphic, JPEG: Joint Photographic Experts Group}
...
\noindent Note that the file path can be a URL. You may, for example store an image in the Dropbox Public folder or on GitHub and use its URL to link to it in the Markdown document.\footnote{For images stored on GitHub use the URL for the raw version of the file} 

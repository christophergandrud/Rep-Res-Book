...
A final tip: if you are using RStudio's \emph{Preview HTML}\index{RStudio!Preview HTML} window to preview your Markdown document, images sourced from a URL will probably not show up. You should preview the document in your web browser instead. This is easy to do in RStudio by clicking on the \texttt{View the page with the system web browser} button (\includegraphics[scale=0.45]{Children/Chapter10/images10/ShowInBrowser.png}). It's located at the top of the \emph{Preview HTML} window.
...
\hlfunctioncall{plot}(x = cars$speed, y = cars$dist,
     xlab = \hlstring{"Speed (mph)"},
     ylab = \hlstring{"Stopping Distance (ft)"},
     cex.lab = 1.5)
@

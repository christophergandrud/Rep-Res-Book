...
\noindent This will open your default web browser and show you an image like that in Figure \ref{GeoMapImage}. If you are using RStudio version 0.98.441 or later the plot will be displayed in a \emph{Viewer} pane within RStudio that is similar to the \emph{Plots} pane. Note that you need to be connected to the internet to view figures created by \emph{googleVis}, otherwise your image will not be able to access the required JavaScript\index{JavaScript} files from the Google Visualization API.\index{API}

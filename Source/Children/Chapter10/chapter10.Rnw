...
Let's briefly look at how to make one type of figure with \emph{googleVis}: a choropleth map. This is created with the \texttt{gvisGeoChart} function.\index{gvisGeoChart}\index{R function!gvisGeoChart} We will use this example to illustrate how to incorporate \emph{googleVis} figures into R Markdown. For demonstrations of the the full range of plotting functions available visit the \emph{googleVis} website: \url{http://code.google.com/p/google-motion-charts-with-r/wiki/GadgetExamples#googleVis_Examples}.
...
\subsection{JavaScript Graphs with \emph{htmlwidgets}-based Pacakges}

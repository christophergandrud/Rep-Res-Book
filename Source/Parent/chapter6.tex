...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcomment{# Gather agricultural methane emissions data from WDI}
AgMethaneData <- \hlfunctioncall{WDI}(indicator = \hlstring{"EN.ATM.METH.AG.ZS"})

\hlcomment{# Show head of AgMethane data frame}
\hlfunctioncall{head}(AgMethaneData)
\end{alltt}
\begin{verbatim}
...
\begin{itemize}
	\item HTML:\index{HTML} Obviously you will encounter a lot of HTML markup when web scraping. Having a good understanding of the HTML markup language will be very helpful. A good, free, resource for learning HTML as well as JSON, JavaScript, XML, and other languages is W3 Schools (\url{http://www.w3schools.com/}).

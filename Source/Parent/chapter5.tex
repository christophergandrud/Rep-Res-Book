...
GitHub is an interface and cloud hosting service built on top of the Git\index{Git} version control system. GitHub was not explicitly designed to host research projects or even data. It was designed to host ``socially coded" computer programs--in what Git calls ``repositories"\index{repository}, repos for short--by making it easy for a number of collaborators to work together to build computer programs. This seems very far from reproducible research.
...
\begin{table}
  ...
  \begin{tabular}{l p{5cm}}
    ...
    \texttt{pull} & \texttt{fetch} data from a remote repository and try to \texttt{merge} it with your commits.
    \hline
  \end{tabular}
  \end{center}
...
Before discussion how to host a Git repository on GitHub it is useful to step back for a second and try to understand what Git is doing when you commit your changes. In the hidden {\emph{.git}} folder Git is saving all of the information in compressed form from each of your commits into a sub-folder called {\emph{Objects}}. Commit objects\index{Git commit object}\footnote{Other Git objects include trees (sort of like directories) and blobs (individual files).} are everything from a particular commit. I mean everything. If you delete all of the files in your repository (except for the {\emph{.git}} folder) you can completely recover all of the files from your most recent commit with the \texttt{checkout} command:
...
Let's learn how to add changes to our GitHub remote repository and access the files based there.

\paragraph{Pushing data to a GitHub}
...
What do \texttt{origin} and \texttt{master} mean? The origin is simply your locally stored repository. ``Master'' refers to the Master or main ``branch'' of the remotely stored repo. We'll look more at branches in the next section. If you have not set up password caching\footnote{See \url{https://help.github.com/articles/set-up-git} for more details.} you will now be prompted to give your GitHub user name and password.
...
What do you do if you and your collaborators are making changes to 

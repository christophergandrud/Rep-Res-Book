...
\hlcomment{# Note: Merged data given the object name CleanedData}
\hlfunctioncall{source}("GatherSource/MergeData.R)
\hlcomment{# Save cleaned/merged Data as MainData.csv}
\hlfunctioncall{write.table}(CleandedData, 
            file = \hlstring{"/DataFiles/MainData.csv"}, 
            sep = \hlstring{","})
\end{alltt}
\end{kframe}
\end{knitrout}
...
Here we saved the output data set {\emph{CleanData}} as a \texttt{.csv} formated file using the {\tt{write.table}}\index{write.table} command. The \texttt{sep} option sets the character that separates values in a row. In our example project, the file {\emph{MainData.csv}} will be the main file we use for statistical analysis.

You can of course save your data in a wide variety of other formats. You can also save all of the objects in your workspace using the \texttt{save.image} command.
...
If you are using RStudio you do the same thing with drop down menus. To open a plain-text data file click on \texttt{Workspace} \textrightarrow \texttt{Import Dataset\ldots} \rightarrow \texttt{From Text File\ldots}. Then specify the separator and other options that will help R understand your data in the box that pops up. This is initially easier than using \texttt{read.table}.

...
**RStudio** is actually as a very nice \LaTeX editor. Even if you aren't creating documents that integrate **R** code, it is still a decent full functioning \LaTeX editor. It has full syntax highlighting, even for documents with with `knitr` code (which it can collapse when you just want to work on the text). It can spell check \LaTeX documents. It can insert common commands like `\section*{}` for unnumbered sections. <!--- Add explanation ---> Most importantly it easily compile \LaTeX documents. <!--- Add full details of how to compile --->   
...
The `write.bibtex` approach is especially nice because it automatically creates a \BibTeX cite key--the packages name. In Chapter 13 we'll look at the `knitcitations` package in more detail.

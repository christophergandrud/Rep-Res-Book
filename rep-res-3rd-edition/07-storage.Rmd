...
We changed the download (`dl`) option from false (`dl=0`) to true (`dl=1`). Now you can use the link to download data in your R source code.
...
many different data formats. Use the `source_url()` \index{R function!source\_url} function in the *devtools* package [@R-devtools] to download and run R source code files (see Chapter \@ref(StatsModel)).
...
compiling an R LaTeX document (see Chapter \@ref(StatsModel)). To have Git ignore particular files, simply create a file called *.gitignore*.[^chapter_5_31] You can either put this file in the repository's parent directory to create a *.gitignore* file for the
...
As we will see in Chapter \@ref([StatsModel) (Section \@ref(sourceurl))

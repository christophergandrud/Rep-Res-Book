...
overall. This will make your workflow more coherent from the beginning
and save you a lot of backtracking later on. Figure \@ref(WorkflowTies)
...
Around the edges of the figure are some of the commands you will learn
to make it easier to go forwards and backwards through the process.
These commands tie your research together. For example, you can use
...
the `read.table()` function can be used to bring this data set into
...
commands, respectively. This helps you maintain a consistent
presentation of results across multiple document types. We'll cover
these commands in detail throughout the book. See Table
\@ref(TableTieCommands) for a brief but more complete overview of the main
*tie commands*.
...
```{r Ch2SessionInfoPlain, size='footnotesize', echo=TRUE, tidy=TRUE}
...
Text files are simple and so, at least at the level of our research documentation
source and markup increases the probability of baseline usefulness in the future. We can extend this logic to all of the tools we use: avoid adding dependencies you don't need to actually gather your data, anylize it, and present the results. Avoid, for example, seeking out additional fonts to use in your presentation slides. I personally have been burned when I wanted to make minor changes to slides a year after I first presented them (and a day before teaching an upcoming class) only to find that they are no longer available; breaking my slides and forcing me to spend considerable time reworking things. If I,.the creator of the slides, found this time consuming, imagine how an outside researcher would find it. 
...
Tie commands are the most dynamic way to explicitly link your files
together. These commands instruct the computer program you are using to
use information from another file. In Table \[TableTieCommands\] I have
compiled a selection of key tie commands you will learn how to use in
...
files accessible through the cloud computing services Dropbox and
GitHub.

...
the nitty-gritty:\@label(OtherBooks)
...
On Linux you will need to install the *RCurl* \citep{R-RCurl}\index{RCurl} and 
*XML*\index{XML} \citep{R-XML} packages separately. Use your 
Terminal\index{Terminal} to install these packages with the following code:
...
function from the *rio* package \citep{R-rio}\index{rio} can be used to bring this data set into
...
a new pane with a bar across the top that looks like the first image in
...
\begin{figure}[ht]
    ...
    \begin{center}
        \begin{subfigure}
            ...
            \label{fig:Rsource}
            \includegraphics[width = \textwidth]{images/chapter_3/RSourceBar.png}
        \end{subfigure}
        \vspace{0.5cm}
        \begin{subfigure}
            \caption{R Markdown Files}
            \label{fig:RMarkdown}
            \includegraphics[width = \textwidth]{images/chapter_3/MarkdownSourceBar.png}
        \end{subfigure}
        \vspace{0.5cm}
        \begin{subfigure}
            \caption{R LaTeX Files}
            \label{fig:RLaTeX}
            \includegraphics[width = \textwidth]{images/chapter_3/LaTeXSourceBar.png}
        \end{subfigure}
    \end{center}
\end{figure}
\setcounter{figure}{3}
...
(see Figure \@(KnitProcess). First we create a knittable markup
...
select `File` `` `R Markdown…`. A window will appear that looks like
...
used with `.Rnw` files. As use of `.Rtex` is currently very low, we won't 
...
eval=FALSE} "'r ChunkLabel, echo=FALSE
string_num_object <- cbind(character_vector, numeric_vector) "' ```
...
\[ChunkOptionsTable\]
  ...
  \[0.25cm\] `cache`             Logical       Whether or not to save results from the code chunk in a cache database. Note: cached chunks are only run when they are changed.
  \[0.25cm\] `cache.vars`    Character Vector  Specify the variable names to save in the cache database.
  \[0.25cm\] `eval`              Logical       Whether or not to run the chunk.
  \[0.25cm\] `echo`              Logical       Whether or not to include the code in the presentation document.
  \[0.25cm\] `error`             Logical       Whether or not to include error messages.
  \[0.25cm\] `engine`           Character      Set the programming language for *knitr* to evaluate the code chunk with.
  \[0.25cm\] `fig.align`        Character      Align figures. (Note: does not work with R Markdown documents.)
  \[0.25cm\] `fig.path`         Character      Set the directory where figures will be saved.
  \[0.25cm\] `include`           Logical       When `include=FALSE` the chunk is evaluated, but the results are not included in the presentation document.
  \[0.25cm\] `message`           Logical       Whether or not to include R messages.
  \[0.25cm\] `out.height`        Numeric       Set figures' heights in the presentation document.
  \[0.25cm\] `out.width`         Numeric       Set figures' widths in the presentation document.
  \[0.25cm\] `results`          Character      How to include results in the presentation document.
  \[0.25cm\] `tidy`              Logical       Whether or not to have *knitr* format printed code chunks.
  \[0.25cm\] `warning`           Logical       Whether or not to include warnings.
  \[0.25cm\]                                   
  ------------------------- ------------------ ---------------------------------------------------------------------------------------------------------------------------------
...
pkg_additional <- c(
    .packages(),
    "bookdown", "knitr", "pacman", "RCurl", "rmarkdown", "styler", "tinytex", "xml"
)

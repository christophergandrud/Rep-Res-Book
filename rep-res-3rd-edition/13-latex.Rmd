# Presenting with *knitr*/LaTeX {#LatexChapter}
...
your results. In the next chapter (Chapter
\@ref(LargeDocs) we will build on these skills by learning how to use *knitr* to create more complex LaTeX documents.
...
package. As we will see in Chapter
\@ref(MarkdownChapter), Markdown syntax is much simpler than
...
customizable formatting needs. Using *kntr* and LaTeX can be useful in
these situations.
...
beamer slideshows with LaTeX and *knitr*.

**Note:** This chapter and the following chapter are unusual for this
book in that they do not refer to both *knitr* and *rmarkdown*. Instead
they focus on capabilities largely exclusive to *knitr*.

## The Basics

In this section we will look at how to create a LaTeX article including
...
As I mentioned earlier, RStudio is a fully functional LaTeX editor in
addition to being an integrated development environment for R. If you
want to create a new LaTeX document you can click in the menu bar then .

![image](images/chapter_11/TeXFormat.png)
...
to compile them you might need to change the file extension from to .
You can also insert many of the items we will cover in this section into
your documents with RStudio's LaTeX `TeX Format` button. See Figure
\@ref(TeXFormat).
...
book in RStudio, I typed much of the LaTeX markup in the Atom
Text[^chapter_11_2] text editor. None of these options have RStudio's high-level integration with *knitr*, however.
...
### The LaTeX preamble & body {- #LaTeXPreamble}
...
`documentclass` command. This specifies what type of presentation
...
This is all you need to get a very basic article class document working.
...
Then it's a good idea to specify title information just after the
...
the date.[^chapter_11_5] Then include the `maketitle` command. This will place your
...
```{r Ch11FullArticle, eval=FALSE, engine='sh'}
%%%%%%%%%%%%%% Article Preamble %%%%%%%%%%%%%%
...
I gathered data from \cite{Pemstein2010} on countries' democracy level. They call
their democracy measure the Unified Democracy Score (UDS). Figure \ref{DemPlot}
shows the mean UDS scores over time for all of the countries in their sample.
...
This document was created using the R version \Sexpr{paste0(version$major, ".",
version$minor)} and the R package \emph{knitr} \citep{R-knitr}. 
It also relied on the R packages \emph{ggplot2} and \citep{R-ggplot2}. 
The document can be completely reproduced from source files available on 
GitHub at: 
...
command in a moment.\index{LaTeX command!emph} Third, using the `thanks` command allows us to create a footnote for author contact information[^chapter_11_6] that is not
numbered like the other footnotes (see below). Finally, you'll notice
`\href{mailto: . . . .org}}`. This creates an email address in the final
document that will open the reader's default email program when clicked.
...
version number into the presentation document.
...
on a page type: `\vspace{3cm}`. This gives us the following space:
...
horizontal space use the `hspace` command in the same way as `vspace`.

### Horizontal lines {-}

Use the `hrulefill` command to create horizontal lines in the text of
your document. For example, `\hrulefill` creates:
...
`\emph{textbf{italicize and bold}}`.
...
code chunk option.
  ...
  : LaTeX Font Size Commands[]{label="LaTeXFontSize"}
...
page on the topic:
<http://en.wikibooks.org/wiki/LaTeX/Special_Characters>. If you
regularly use non-English alphabets you might also be interested in
reading the LaTeX Wikibook page on on internationalization:
...
mathematicians because it is very good at rendering mathematics. A
...
item is delimited with the `item` command. For example:
...
`itemize`. You can create sublists simply by nesting lists inside of
lists like this:
...
the text. Then type the footnote's text. Of course, remember to close
...
reference, add a `label` such as `\label{ACrossRefLabel}`. It doesn't
really matter what label you choose, though make sure they are not
duplicated in the document. Also, it can be a good idea to use the same
conventions that we learned for labeling R objects (see Section
\@ref(ObjectNames). Then place a `ref` command (e.g.
...
will place the heading number. If `label` is in a `table` or `figure`
environment you will get the table or figure number. You can also use
`pageref` instead of `ref` to include the page number. Finally, loading
the *hyperref* package makes cross-references (or footnote) clickable.
...
citation can and needs to include. Then we have the `CITE\_KEY`. This is
...
Each citation must have a unique `CITE\_KEY`. A common way to write
...
BibTeX file is called *Main.bib* and it is in the same directory as your
markup document, then type: `\bibliography{Main.bib}`. You can use a
...
using the `bibliographystyle` command. For example, this book uses the
...
Place the `cite` command in your document's text where you want to place
a reference. You include the `CITE\_KEY` for the reference in this
command, e.g. `\cite{donoho2009}`. You can include multiple citations in
`cite`, just separate the `CITE\_KEY`s with commas. You can add options
...
commands to change what information is included in the parentheses. The following table contains a selection of these commands and examples:
  ...
  ---------------------------- ----------------
  `\cite{donoho2009}`             [@donoho2009]
  `\citep{donoho2009}`            [@donoho2009]
  `\citeauthor{donoho2009}`         @donoho2009
  `\citeyear{donoho2009}`        [-@donoho2009]
  `\citeyearpar{donoho2009}`     [-@donoho2009]

  : A Selection of *natbib* In-text Citation Style
  Commands[]{label="NatbibTable"}
...
`citation` command inside of a code chunk. For example, if you want the
citation information for the `xtable` package you simply type:
...
the `write_bib` function. Let's make a BibTeX file called *Packages.bib*
containing citation information for the *xtable* package.

```{r Ch10OneBib, eval=FALSE, tidy=FALSE}
# Create package BibTeX file
...
format `R-PACKAGE_NAME`, e.g. `R-xtable`.

**Warning:** *knitr*'s `write_bib` command currently does not have the
...
name, it will overwrite the file. So, be very careful using this command
...
and cite. The next command is `lapply` (list apply). This applies the
function `library` to all of the items in *packages_used*.
...
`library` up until now). If you include these commands in a code chunk
...
figure on a slide, for example. *knitr* also makes it easy to
...
be made when working with beamer and *knitr*. A full example of a
knittable beamer presentation with illustrations of the many of the
points discussed here is printed at the end of the chapter.

![Knitted Beamer PDF
Example[]{label="BeamerExample"}](images/chapter_11/BeamerExample.png){#BeamerExample}

The presentation in this example was created using a custom beamer theme
available at:
<https://GitHub.com/christophergandrud/Make-Projects/tree/master/Rnw_Lecture>.
...
In Chapter \@ref(MarkdownChapter) we will see how to use the *rmarkdown*
package to create beamer presentations with the much simpler Markdown
syntax.
...
how to create more complex LaTeX documents, including theses, books, and
batch reports.

[^chapter_11_1]: Wikipedia has collated a table that comprehensively compares many of these editors: <http://en.wikipedia.org/wiki/List_of_text_editors>.

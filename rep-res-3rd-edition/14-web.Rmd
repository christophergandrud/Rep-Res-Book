# Presenting on the Web and Other Formats with R Markdown {#MarkdownChapter}

LaTeX is the standard markup language for creating academic-quality
articles and books. If we want to present research findings via the
internet, our best option is HTML. HTML syntax can be tedious to write,
as we saw in Chapter \@ref(TablesChapter). Luckily the Markdown language was created as a simplified way of writing HTML documents. As we have seen, Markdown
can be fully integrated with *knitr*/*rmarkdown* for creating
reproducible research HTML presentation documents. In addition, the
*rmarkdown* package allows us to write documents in Markdown and render
them to PDF and MS Word.
...
\@ref(StatsModel), tables (Chapter \@ref(TablesChapter), and figures (Chapter \@ref(FiguresChapter)) with R Markdown documents. In this chapter
...
styles with CSS. Then we will learn how to create HTML slideshows. We'll
finish up the chapter by looking at options for publishing
Markdown-created HTML documents, including locally on your computer,
Dropbox, and GitHub Pages.
...
between HTML element tags. Pandoc and *rmarkdown* have extended Markdown
...
that are of types supported by the output format. You are not be able to
include a JavaScript plot directly in a PDF.

### Getting started with Markdown editors

![image](images/chapter_web/rmarkdownOutputOptions.png)
...
above drop-down menu. This allows you to also compile the
...
anything like that. HTML head elements (HTMLs preamble equivalent) are
added automatically when you render Markdown documents into HTML. So
with Markdown, you can just start typing.
...
page in Markdown, simply place three or more equal signs or dashes
...
You might have noticed that in the headers example we did not need to
separate the header with a blank line.
...
To *italicize* a word in Markdown, simply place it between two
...
To create hyper-links in Markdown, use the `[LINK_TEXT](URL)`
syntax.[^chapter_web_4] `LINK_TEXT` is the text that you would like to show up as the hyper-link text. When you click on this text it will take you to the linked site specified by `URL`. If you want to show only a URL as the
...
hyper-linked. In regular Markdown place the URL between less-than and
...
To create itemized lists in Markdown, simply place the items after one
dash:
...
LaTeX-style equations by adding on the MathJax JavaScript engine.
MathJax syntax is the same as LaTeX syntax (see Section
[\[MathLaTeX\]](#MathLaTeX){reference-type="ref"
reference="MathLaTeX"}), especially when used from RStudio or when
rendered with *rmarkdown*. Markdown documents rendered in RStudio
...
in LaTeX, e.g. `\( s^{2} = \frac{\sum(x - \bar{x})^2}{n - 1} \)`.
...
will learn how to overcome these limitations with Pandoc and CSS via
*rmarkdown*.

### More on *rmarkdown* Headers {-}

In Chapter \@ref(GettingStartedRKnitr) we first saw an *rmarkdown*
...
include these at the top level, i.e. without being tabbed. *rmarkdown*
...
Remember from Chapter \@ref(GettingStartedRKnitr) that we can specify rendering instructions for multiple output formats in the same header. Here is a longer header, building on what we just saw. We'll go through it in detail:
...
built-in ability to use a range of themes from Bootswatch\index{Bootswatch}
(<https://bootswatch.com/>). Alternatively, you can link to a custom CSS
...
[\[PandocCitations\]]{#PandocCitations label="PandocCitations"}
...
footnote to be located use: `[^chapter_web_NOTE_KEY]`. Then at the end of your
document, place `[^chapter_web_NOTE_KEY]: The footnote text`.[^chapter_web_9]
...
To sum up, here is an example of document that can be rendered in HTML
or PDF using *rmarkdown*. It includes footnotes and a bibliography.
...
abilities see: <http://johnmacfarlane.net/pandoc/README.html>.
...
for this is Google Chrome's Developer Tools\index{Google Chrome!Developer Tools} The Developer Tools allows
you to edit your webpages, including their CSS, and see a live preview.
It is a really nice way to experiment with CSS (and HTML and
JavaScript).\index{HTML}\index{JavaScript}[^chapter_web_10] There are also numerous pre-made style sheets
available online.[^chapter_web_11]

##### Rendering R Markdown files to HTML using custom CSS {-}

The simplest way to use a custom CSS style sheet is to include the file
...
`css` option. If our custom CSS file is called *Markdown.css* in the
...
-   HTML presentations can incorporate virtually any content that can be
    ...
    motion charts created by *googleVis*\index{googleVis} (see Chapter \@ref(FiguresChapter)).
...
This code creates a slide show that begins with the slide in Figure
[1.2](#BasicIO){reference-type="ref" reference="BasicIO"}. Bullet points
...
![*rmarkdown*/IO Slides Example Title
...
menu on the left of the window (see following figure). Finally, click `HTML (ioslides)` or
`HTML (Slidy)`.

![Create New *rmarkdown* Presentation in
...
An *rmarkdown* beamer presentation uses the same syntax that we just saw with HTML presentations. The main difference is in the header where we
...
respectively. For example, to create the slide show that begins with the
slide in Figure [1.4](#rmarkdownBeamerExample){reference-type="ref"
reference="rmarkdownBeamerExample"}:
...
Slide[]{label="rmarkdownBeamerExample"}](images/chapter_web/rmarkdownBeamerExample.png){#rmarkdownBeamerExample
...
Just type the slide's title then at least three equal signs (`===`). For example,
...
To compile the slideshow, either click the `Preview` button
(![image](images/chapter_web/PreviewButton.png)) or save the
...
Presentations, create a standalone HTML file by simply clicking the
...
In Chapter \@ref(GettingStartedRKnitr) we saw how to publish other R Markdown documents compiled with RStudio to RPubs. The *knitr* function `knit2wp`\index{knitr!knit2wp}
...
Of course, you can simply open the HTML file rendered from any R
Markdown document in your web browser. If the HTML file contains the
full information for the page as they generally do when created by
*rmarkdown*, e.g. the file does not depend on any auxiliary files, you
can simply share this file via email or whatnot and anyone with a web
...
```{r Ch13GHPages1, eval=FALSE, tidy=FALSE, engine='sh'}
# Create orphan
gh-pages branch git checkout --orphan gh-pages
```

Now `add` the files, `commit` the changes and `push` it to GitHub. Push
it to the *gh-pages* branch like this\index{Git command!add\index{Git command!commit}\index{Git command!push}

```{r Ch13GHPages2, eval=FALSE, tidy=FALSE, engine='sh'}
# Add files
git add .
...
We have covered many of the core capabilities of *rmarkdown* for
...
presentation is the *shiny* package [@R-shiny].\index{shiny} It gives R the
...
your research. *rmarkdown* also makes it easy to create documents in a
variety of formats using the simple R Markdown format. For relatively
simple documents this can be a very useful tool.
...
[^chapter_web_14]: <http://www.w3.org/Talks/Tools/Slidy2/#(1)>
...
[^chapter_web_17]: <http://wordpress.com>

[^chapter_web_19]: An orphan branch is a branch with a different root from other
    repository branches. Another way of thinking about this is that they

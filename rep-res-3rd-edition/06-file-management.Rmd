...
are subdirectories of the root directory.\index{root directory}
...
be further subdirectories and so on. A directory inside of another
...
Below we'll see how to RStudio Projects and the *here* [@R-here] package to build on your relative file paths to further enhance easy reproducibility.
...
One approach is to use a convention that contrasts with the R object naming convention you are using to distinguish between object and file names. For example, if we adopt the underscore method used in Chapter \@ref(GettingStartedRKnitr) (e.g. `health_data`) we could use em dashes (`-`) to separate words in file names. For example, `example-source.R`.
...
## Organising Research with RStudio Projects {#CreateRStudioProject}
...
### Set working directories with *here*
...
Again, I have a lot of files in my working directory, so I will shorten the output by piping it through the command line's `head` command.\index{shell command!head} The command line pipe is not `%>%` as in R, but instead `|`.\index{shellcommand!|}
...
````bash
mv /example-project/ExampleEcho.md /markdown-files
````

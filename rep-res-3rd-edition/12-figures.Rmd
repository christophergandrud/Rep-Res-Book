...
(page ). We can then include a `caption` and `label` function.\index{LaTeX!label}\index{LaTeX!caption} The
...
in Chapter \@ref(MarkdownChapter) as this is what it is intended to imitate.
...
Markdown does not easily include ways to resize or re-position an image. 
If you want to resize or position
your image it is often most straightforward use HTML markup. Probably the simplest way
...
For example, in a code chunch we could place:
...
Notice in the code chunk options `fig.cap`. In the next section we dicuss these types of *knitr* options in detail.

...
actions to objects. We will talk about the actions–functions–later in this section.[^chapter3_4] Now let's create a few objects.
...
contain spaces.[^chapter3_5] 

Your code will be much easier to read if object 
names are short and meaningful. It is also a good idea to give each object a 
unique name to avoid confusion and conflicts. Make sure that object names are 
different from the names of their components, e.g. individual variable names. 
This will avoid many complications like accidentally overwriting an object or
confusing R about what object or component you are referring to.

Let's call our numeric object *number*. To put something into the object we use 
the assignment operator[^chapter3_6]
...
Finally, `[1]` is the row number of the object that 10 is on. Our object only 
has one row.
...
from. Nonetheless, `attach()` and `with()` can be useful for
...
You can stack functions inside of arguments. To have R find the
...
[^chapter3_3]: If you are using a Unix-like system such as Linux Ubuntu or Mac OS
    ...
    and then the `Return` or `Enter` key. This will begin a new R session. You will know
...
[^chapter3_5]: @whickham2014book argues that underscores (`_`) should be used 
    to separate words in object names. This appears to becoming the dominat convention in R. 
    Other conventions are to use either periods (`.`) or capital

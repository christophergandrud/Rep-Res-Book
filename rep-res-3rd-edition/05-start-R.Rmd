...
`source` function. Whe you click on the last icon on the right (it has a series of stacked lines) you will get a navigable table of contents for your file; very useful for working 
...
Figure \@ref(fig:rmarkdownWindow). In this windo select the type of output document
...
`ex_label`. If we want to run thecode chunk, but
...
[^chapter3_5]: @whickham2014book argues that underscores (`_`) should be used 
    ...
    Underscores appear to now be the dominat naming convention in the R community. 
...
[^chapter3_29]: Note that you can also create an *rmarkdown* document without a
    header. *tmarkdown* will just use the default settings when knitting.  

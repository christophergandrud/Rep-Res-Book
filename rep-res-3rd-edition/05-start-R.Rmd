...
panel (what appears when you open RStudio) and some of its main tools
...
Use the assignment operator[^chapter3_6]
(`<-`)\index{R function!<-} to put something into the object:
...
Knowing how to use vectors will be especially helpful when you clean up
raw data in Chapter \@ref(DataClean) and make tables in Chapter
\@ref(TablesChapter).[^chapter3_10]

#### Vectors {-}

Vectors are the "fundamental data type" in R [@matloff2011]. They are an ordered group of numbers, character strings, and so on.[^chapter3_11]
It may be useful to think of most data in R as composed of
vectors. For example, data frames\index{R function!data.frame}\index{R!data frame} are basically collections
...
files. They are not text files. They are not human readable. They are much less future-proof.
...
When you execute code in the R console, it becomes part of your history. Use the `history()` function\index{R function!history} to see
...
document's markup which is the text and rules for how to format the text. *knitr*
...
multiple formats from a knittable document written in Markdown. The
...
In addition to the header, R Markdown differs from basic knitr files in that you
...
results. Perhaps we don't want to show the code at all but just a
...
figures to be saved (see Chapter \@ref(DirectoriesChapter)).
...
In this chapter's appendix we discuss interactive Jupyter notebooks.\index{Jupyter} They are popular in the data science and tech industries and use a somewhat different logic from R Markdown notebooks.
...
before, but instead of clicking a `Complie PDF` or `knit HTML` button, use the `knit()` function. To knit a
...
Then you `knit()`\index{R function!knit} the file:
...
run the *.md* file through the `markdownToHTML()`\index{R function!markdownToHTML}
...
If we want to compile a *.tex* file in R, we run it through the `texi2pdf()` function\index{R function!texi2pdf} from the
...
to use RStudio, and knitr/R Markdown syntax for multiple markup
languages. These tools, especially R and knitr/R Markdown, are
...
[^chapter3_2]: This figure and almost all screenshots in this book were taken on
    a computer using the macOS 10.14 operating system.

[^chapter3_3]: If you are using a Unix-like system such as Linux Ubuntu or macOS, you can also access R via an application called the Terminal. If
...
[^chapter3_4]: Functions are also objects. In this chapter, I treat them as distinct from other object types to avoid confusion.
...
[^chapter3_5]: @whickham2014book argues that underscores (`_`) should be used
    ...
    The underscore object naming convention appears to now be the dominant
    style in the R community. There are other conventions. These include using
    periods (`.`) or capital letters (referred to as CamelBack) to separate words. For more information on R naming conventions, see @baath2012.
...
[^chapter3_28]: If you are only running one line of code, you don't need to
    highlight the code; you can put your cursor on that line.
...
[^chapter3_33]: The "nw" refers to the noweb simple literate programming tool that
    Sweave built on [@leisch2002; @ramseynoweb].

...
your document.[^chapter_9_24] This makes it so that you don't need to
...
the bottom of the table, as it is in Table \@ref(BasicXtableSummary), to the top. See the *xtable* package
...
models in LaTeX as the next table. For example, to estimate nested
...
```{r Ch9BasictexregTable1Create, echo=FALSE, results='asis', message=FALSE}
library(texreg)
...
![](images/chapter_9/htmlregExample.png)
...
model: \index{R package!brms}\index{R function!brm}
...
clicking on them in RStudio's *Environment* tab.
...
It is convenient to include the creation of this table in your
...
present it in LaTeX and Markdown documents with knitr/R Markdown. In
...
[^chapter_9_2]: For example, in a replication of Reinhart and Rogoff's [-@rr2010] much cited
    study of economic growth and public debt, @herndon2014
...
[^chapter_9_23]: Note: you can abbreviate `print.xtable()` as `print()`.

[^chapter_9_24]: Of course, you will probably want to use the `include=FALSE`
    *knitr* option with this code chunk.

[^chapter_9_25]: <https://cran.r-project.org/web/packages/xtable/xtable.pdf>

...
computational research [@Fomel2009;
```Buckheit1995]. Rob Hyndman more
...
using the `read.table()` function.\index{R function!read.table} For example, imagine we have a CSV file
called *test-data.csv* stored in the current working directory. To load
...
the *foreign* package supports, use the following command:\index{R function!read.dta}
...
`https` rather than `http`. We have to use different functions to
download data from secure URLs. Let's look at two methods for
downloading data into R: `import()` and the *RCurl* package.

##### Loading data from secure URLs with  {- #SecureData6 #DropboxNonPublic}

Files stored on Dropbox are a little trickier to
...
```{r Ch6sourceDropboxData, tidy=FALSE}
# Download data from Dropbox
FinDataFull <- rio::import("fin_research_note.csv", "exh4iobbm2p5p1v",
                           format = "csv")
```
...
rely on *repmis* is to use the `getURL()` function\index{R function!getURL} in the *RCurl* package
[@R-RCurl] as well as `read.table()`\index{R function!read.table} and `textConnection()`.\index{R function!textConnection} The latter
...
\@ref(GitDownload into R we could use this code:

```{r Ch6httpsLoad, message=FALSE, tidy=FALSE, size='small'}
# Put URL address into the object url_address
url_address <- paste0("https://raw.githubusercontent.com/",
    "christophergandrud/Disproportionality",
    "_Data/master/Disproportionality.csv")

# Download Electoral disproportionality data
data_url <- RCurl::getURL(url_address)

# Convert Data into a data frame
disprop_data <- read.table(textConnection(data_url), sep = ",", header = TRUE)

# Show variables in the data
names(disprop_data)
```

If running `getURL(url_address)` gives you an error about an
...
```{r Ch6WDIFertSave, include=FALSE}

...
compile information from paper based archives, for example. Data hosted online may disappear 
when the host ceases operation. The best you
...
Let's see what an R data make file might look like for a project with a file
...
Dropbox, GitHub, and Dataverse store their data at secure URLs.[^chapter_6_16] You can tell if the data is stored at a secure web address if it begins with
...
-   download the file with `download.file()`,\index{R fuction!download.file}

-   decompress the file with one of the commands in base R,[^chapter_6_22]

-   read the file with `read.csv()` or `import()`.\index{R fuction!read.csv}\index{R fuction!import}
...
Note I used `paste0()`\index{R function!paste0} simply to split the URL over two lines so I could print the whole URL on this page.
...
The complexity of this process depends on how structured the data is. If the data is in a CSV file then all we need the `import()` function. Less structured data requires more effort to download and parse. For example, data
...
-   Looping: Web scraping often involves applying a function to multiple
    ...
    a comprehensive overview. The *dplyr* [@R-dplyr] and *purrr* [@R-purrr] for data frame and vector
...
[^chapter_6_3]: We use the command more in the Chapter \@ref(StatsModel).
...
[^chapter_6_14]: See his blog at:
    <https://robjhyndman.com/hyndsight/makefiles/>. Posted 31 October

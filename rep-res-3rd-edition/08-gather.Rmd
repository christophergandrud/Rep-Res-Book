...
reference="DirectoriesChapter"}, simply tells R where to look for and
...
structure similar to the example project in Figure
[\[ExampleTree\]](#ExampleTree){reference-type="ref"
reference="ExampleTree"}. The file paths in this example are for
...
extension `.R`, i.e. our data gathering and merge source code files.
...
"wildcard",[\[AsteriskWildcard\]]{#AsteriskWildcard
label="AsteriskWildcard"} a special character that allows you to select
...
If we run the following code:[\[MakeAllCommand\]]{#MakeAllCommand
label="MakeAllCommand"}

\<\<Ch6MakeAll, eval=FALSE, engine='sh', echo=TRUE\>\>= \# Remove
merge-data.Rout and make all R source files make cleanMerge all @
...
*Environment/History* pane, the same place where the *Git* tab appears
(see Figure [1.1](#BuildTab){reference-type="ref"
reference="BuildTab"}).[^chapter_6_13]
...
`make clean all`). As you can see in Figure
[1.1](#BuildTab){reference-type="ref" reference="BuildTab"}, the *Build*
tab shows you the same output you get in the shell.

![The RStudio Build
Tab[]{label="BuildTab"}](Children/Chapter6/images6/BuildTab.png){#BuildTab}
...
using the `read.table` command. For example, imagine we have a CSV file
called *TestData.csv* stored in the current working directory. To load
the data set into R simply type:

\<\<Ch6LocalReadTable, eval=FALSE, tidy=FALSE, echo=TRUE\>\>= TestData
\<- read.table(\"TestData.csv\", sep = \",\", header = TRUE) @

See Section [\[SepHeadExplain\]](#SepHeadExplain){reference-type="ref"
reference="SepHeadExplain"} for a discussion of the arguments in this
command.
...
example, imagine we have a data file called *Data1.dta* stored in our
working directory. This file was created by the Stata statistical
program. To load the data into an R data frame object called *StataData*
simply type:

\<\<Ch6Stata, eval=FALSE, tidy=FALSE, echo=TRUE\>\>= \# Load foreign
package library(foreign)

\# Load Stata formatted data StataData \<- read.dta(file =
\"Data1.dta\") @

As you can see, commands in the *foreign* package have similar syntax to
`read.table`. To see the full range of commands and file formats that
the *foreign* package supports, use the following command:

\<\<Ch6ForeignHelp, eval=FALSE, echo=TRUE\>\>= library(help =
\"foreign\") @

If you have data stored in a spreadsheet format such as Excel's *.xlsx*,
...
your `read.table` command.

### Data from secure () URLs {#SecureDataDownload}

Storing data at non-secure URLs is becoming less common. Services like
Dropbox and GitHub now store their data at secure URLs.[^chapter_6_16] You can
tell if the data is stored at a secure web address if it begins with
`https` rather than `http`. We have to use different commands to
download data from secure URLs. Let's look at three methods for
downloading data into R: `source_data`, `source_DropboxData`, and the
*RCurl* package.

##### Loading data from secure URLs with  {#SecureData6}

As we saw in Chapter [\[Storing\]](#Storing){reference-type="ref"
reference="Storing"}, we can use the `source_data` command in the
*repmis* package to simplify the process of downloading data from
Dropbox *Public* folders (Section
[\[EnablePublicFolder\]](#EnablePublicFolder){reference-type="ref"
reference="EnablePublicFolder"}) and GitHub (Section
[\[GitDownload\]](#GitDownload){reference-type="ref"
reference="GitDownload"}). You can use `source_data` to download data in
plain-text format from almost any URL, as long as the file is not
embedded in a larger HTML website.

One problem for reproducible research with sourcing data located on the
internet is that data files may change without us knowing. This could
change the results we get. Luckily, we can solve this problem with
`source_data`. In Chapter [\[Storing\]](#Storing){reference-type="ref"
reference="Storing"} we saw that when we run the `source_data` command
we not only download a data file, but also find its SHA-1 hash. The
SHA-1 hash is basically a unique number for the file. If the file
changes, its SHA-1 hash will change. Once we know the file's SHA-1 hash
we can use `source_data`'s `sha1` argument to make sure the file that we
downloaded is the same as the one we intended to download.

For example, let's find the SHA-1 hash for the disproportionality data
set we downloaded in the last chapter (Section
[\[GitDownload\]](#GitDownload){reference-type="ref"
reference="GitDownload"}):[^chapter_6_17]

\<\<Ch6sourceDataGitHub, tidy=FALSE, size='small', message=TRUE\>\>=
DispropData \<- rio::import(UrlAddress) @

##### Loading data from Dropbox non-Public folders with  {#DropboxNonPublic}

Files stored on Dropbox non-*Public* folders are a little trickier to
download. If you go to the Dropbox website and click the `Share Link`
button next to a file
(![image](Children/Chapter5/images5/DropboxLink.png)) you will be given
an information box. This is not the raw data file. Luckily, *repmis*
includes a `source_DropboxData` command for downloading data stored in a
non-Public Dropbox folder into R. It works in much the same way as
`source_data`, the only difference is that instead of using the URL we
need (a) the file's name and (b) its Dropbox key.

To find the file's key simply click on the `Share Link` button next to
the file on the Dropbox website. Look at the URL for the webpage that
appears. Here's an example:
<https://dl.dropboxusercontent.com/s/exh4iobbm2p5p1v/fin_research_note.csv>

You can see that the last part of the URL (`fin_research_note.csv`) is
the data file's name. The key is the string of letters and numbers just
after `https://www.dropbox.com/s/`, i.e. `exh4iobbm2p5p1v`. Now that we
have the file name and key we can download the data into R using
`source_DropboxData`. For example:

\<\<Ch6sourceDropboxData, tidy=FALSE\>\>= \# Download data from a
Dropbox non-Public folder FinDataFull \<-
rio::import(\"fin\_research\_note.csv\", \"exh4iobbm2p5p1v\", format =
\"csv\") @

##### Loading data using

A more laborious way to download data from a secure URL that does not
rely on *repmis* is to use the `getURL` command in the *RCurl* package
[@R-RCurl] as well as `read.table` and `textConnection`. The latter
commands are in base R. The two rules about data being stored in plain
text-formats and not being embedded in a larger HTML website apply to
this method as well.

Let's try an example. To download the data file we used in Section
[\[GitDownload\]](#GitDownload){reference-type="ref"
reference="GitDownload"} into R we could use this code:

\<\<Ch6httpsLoad, message=FALSE, tidy=FALSE, size='small'\>\>= \# Put
URL address into the object UrlAddress UrlAddress \<-
paste0(\"https://raw.githubusercontent.com/\",
\"christophergandrud/Disproportionality\",
\"\_Data/master/Disproportionality.csv\")

\# Download Electoral disproportionality data DataUrl \<-
RCurl::getURL(UrlAddress)

\# Convert Data into a data frame DispropData \<-
read.table(textConnection(DataUrl), sep = \",\", header = TRUE)

\# Show variables in the data names(DispropData) @

If running `getURL(UrlAddress)` gives you an error about an
`SSL certificate problem` simply add the argument
`ssl.verifypeer = FALSE`. This allows you to skip certification
verification and access the data.[^chapter_6_18]
...
Let's download a compressed file called *uds\_summary.csv* from
[@Pemstein2010]. It's in a compressed file called *uds\_summary.csv.gz*.
...
size='footnotesize'\>\>= \# For simplicity, store the URL in an object
called 'URL' URL \<-
\"http://www.unified-democracy-scores.org/files/20140312/z/uds\_summary.csv.gz\"

\# Create a temporary file called 'temp' to put the zip file into. temp
\<- tempfile()

\# Download the compressed file into the temporary file.
download.file(URL, temp)

\# Decompress the file and convert it into a data frame UDSData \<-
read.csv(gzfile(temp, \"uds\_summary.csv\"))

\# Delete the temporary file. unlink(temp)

\# Show variables in data names(UDSData) @
...
information becomes available. Some examples of these packages include:

-   The *openair* package [@R-openair], which beyond providing a number
    of tools for analyzing air quality data also has the ability to
    directly gather data directly from sources such as Kings College
    London's London Air (<http://www.londonair.org.uk/>) database.

-   The *quantmod* package [@R-quantmod] allows you to access data from
    Google Finance,[^chapter_6_23] Yahoo Finance,[^chapter_6_24] and the US Federal
    Reserve's FRED[^chapter_6_25] economic database.

-   The *treebase* package by [@Boettiger2012] allows you to access
    phylogenetic data from TreeBASE.[^chapter_6_26]

-   The *twitteR* package [@R-twitteR] accesses Twitter's[^chapter_6_27] API. This
    allows you to download data from Twitter including tweets and
    trending topics.

-   The *WDI* package [@R-WDI] allows you to directly download data from
    the World Bank's Development Indicators database.[^chapter_6_28] This database
    includes numerous country-level economic, health, and environment
    variables.

-   The rOpenSci[^chapter_6_29] group has and is developing a number of packages
    for accessing scientific data from web-based sources with R. They
    have a comprehensive set of packages for accessing biological data
    and academic journals. For a list of their packages see:
    <http://ropensci.org/packages/index.html>.

-   Stack Exchange's Cross Validated website[^chapter_6_30] also has a fairly
    comprehensive and regularly updated list of APIs accessible from R
    packages.
...
\<\<Ch6WDIsearch\>\>= \# Load WDI package library(WDI)
...
AG.CON.FERT.ZS. We can use the command `WDI` to gather the data and put
it in an object called *FertConsumpData*.

\<\<Ch6WDIFert, eval=TRUE\>\>= FertConsumpData \<- WDI(indicator =
\"AG.CON.FERT.ZS\") @

The data we downloaded looks like this:

\<\<Ch6WDIFertSave, include=FALSE\>\>= \# This ensures that the PDF will
still compile even if the data is unavailable via WDI \#load(file =
\"Source/Children/Chapter6/FertData.RData\") @

\<\<Ch6HeadFert\>\>= head(FertConsumpData) @
...
`getURL` command. We then parsed the downloaded text as a CSV formatted
...
commands such as `readHTMLTable` for parsing and extracting this kind of
...
you can read it with the *rjson* [@R-rjson] or *RJSONIO* [@R-RJSONIO]
packages.
...
may be useful. It is a wrapper for *RCurl* intended to make accessing
APIs easier.
...
[^chapter_6_4]: GNU Make was originally developed in 1977 by Stuart Feldman as a
    ...
    instructions please see Section
    [\[InstallMake\]](#InstallMake){reference-type="ref"
    reference="InstallMake"}.
...
[^chapter_6_16]: Dropbox used to host files in the Public folder at non-secure
    URLs, but switched to secure URLs.

[^chapter_6_17]: Remember we placed the file's raw GitHub URL address inside of
    the object *UrlAddress*.

[^chapter_6_18]: For more details see the *RCurl* help page at

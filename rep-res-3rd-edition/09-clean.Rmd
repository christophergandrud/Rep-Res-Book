...
We can order observations in our data set using the `order()` function.\index{R function!order}\index{sort}\index{order} For example, to order *fert_long* by year-country we type:
...
We can use the `subset()`\index{subset}\index{R function!subset} function to examine these outliers,\index{outliers} for example, countries that have fertilizer consumption greater-than 1000 kilograms per hectare.
...
# Drop Arab World type from fert_long_sub
...
*dbplyr* [@R-dbplyr] or *data.table* packages [@R-data.table]. They have many

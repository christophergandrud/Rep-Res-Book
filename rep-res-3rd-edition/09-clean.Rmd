...
# Gather fert_spread
fert_gathered <- gather(fert_spread, Year, Fert, 3:9)
...
fert_gathered <- rename(fert_gathered,
                    year = Year,
                    fert_cons = Fert)
...
```{Ch7Arrange}
fert_gathered <- arrange(fert_gathered, country, year)
```
...
FertOutliers <- subset(x = fert_gathered,
                      fert_cons > 1000)
...
fert_gathered_sub$country[fert_gathered_sub$country == "Korea, Rep."] <- "South Korea"
```
...
fert_gathered_sub$fert_cons_log <- log(fert_gathered_sub$fert_cons )
...
fert_gathered_sub$fert_cons[fert_gathered_sub$fert_cons == 0] <- 0.001
...
fert_gathered_sub$fert_cons_group[fert_cons >= 18 & fert_cons < 81] <- 2
fert_gathered_sub$fert_cons_group[fert_cons >= 81 & fert_cons < 158] <- 3
...
fert_gathered_sub$fert_cons_group <- factor(fert_gathered_sub$fert_cons_group, 
                                            labels = fc_labels)
...
Afghanistan-2004 in the other) we need to add the `union()`\index{R function!union} function to
`merge`'s `by` argument. Here is a full example:[^chapter_7_7]

```{r Ch7MergeFull, tidy=FALSE}
# Merge fin_regulator and disprop_data
merged_data_2 <- merge(fin_regulator, disprop_data, union("iso2c", "year"), 
                     all = TRUE)

# Merge combined data frame with fert_gathered_sub 
merged_data_2 <- merge(merged_data_2, fert_gathered_sub, union("iso2c", "year"), 
                     all = TRUE)
...
data_duplicates <- merged_data_2[duplicated( merged_data_2[, 1:2]), ]
...
data_not_duplicates <- merged_data_2[!duplicated(merged_data_2[, 1:2]), ]
```
...
# Rename country.x = country final_cleaned <-
dplyr::rename(final_cleaned, country = country.x)
```

```{r Ch7Showfinal_cleanedNames} 
# Show final_cleaned variables
names(final_cleaned)
```
...
# Spread fert_cons_data to year wide format fert_spread <-
...
R code for creating Figure \@ref(fig:fert_consDens):
...
[^chapter_7_long_format]: For an excellent discussion of ideal data formats see [@wickham2014article].
...
[^chapter_7_1]: The *countrycode* package
...
[^chapter_7_7]: You can download a modified version of this example as part of the
